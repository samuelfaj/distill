...
This crate extracts a first-class `Agent` type from `distill-shell`.
...
`distill-shell`, another in-process host, or a headless batch runner.
...
```rust
use distill_agent::{AgentDefinition, AgentBuilder};
...
4. **Built-in**: `default_distill()`, `browser_use()`
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  distill-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`distill-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `distill-agent` depends on it for tool setup.
- **`distill-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `distill-agent` (AGENTS.md
...
|---|---|---|
| `Distill` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p distill-agent

# Test
cargo test -p distill-agent

# Clippy
cargo clippy -p distill-agent --fix --allow-dirty
