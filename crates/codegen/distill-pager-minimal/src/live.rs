...
use distill_pager::app::PagerTerminal;
use distill_pager::app::app_view::{ActiveView, AppView};
use distill_pager::minimal_api;
use distill_pager::render::Renderable;
use distill_pager::scrollback::state::ScrollbackState;
use distill_pager::scrollback::wrappers::EntryRenderer;
use distill_pager::terminal::TerminalContext;
use distill_pager::theme::Theme;
use distill_pager::views::prompt_widget::{PromptBg, PromptStyle};
use distill_pager::views::turn_status;
...
/// edge.
pub(super) fn live_left_inset(_appearance: &distill_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut distill_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        distill_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub(super) fn prompt_style(
    appearance: &distill_pager::appearance::AppearanceConfig,
    input_mode: distill_pager::app::agent_view::PromptInputMode,
    theme: &Theme,
    multiline: bool,
) -> PromptStyle {
    PromptStyle {
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal, ctx: &TerminalContext) {
    ...
    distill_pager::render::draw::draw_frame(terminal, cursor, ctx, |frame, _link_spans| {
        let area = frame.area();
        ...
        agent.active_pane = distill_pager::app::agent_view::AgentPane::Prompt;
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                distill_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
            distill_pager::views::btw_overlay::btw_panel_height(
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            distill_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(distill_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a distill_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &distill_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_advance_phase_timer(
    agent: &mut distill_pager::app::agent_view::AgentView,
) -> Option<distill_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &distill_pager::app::agent_view::AgentView,
    activity: &Option<distill_pager::acp::tracker::TurnActivity>,
    ...
    turn_status::render_turn_status(
        ...
        turn_status::TurnStatusArgs {
            ...
            goal_verifying,
            watchers,
            jev: distill_shell::jev::turn_activity(agent.turn_started_at),
            parked,
...
fn render_config_status_line(
    ...
    area: Rect,
    agent: &mut distill_pager::app::agent_view::AgentView,
    frame: &distill_pager::views::status_line::StatusLineFrame,
    theme: &Theme,
) {
    if area.height == 0 || area.width == 0 {
        return;
    ...
    if let Some(width) = minimal_api::status_line_inner_width(area.width, padding) {
        agent.last_status_line_size = Some(distill_pager::views::status_line::RowSize {
            cols: width,
            lines: area.height,
        });
    }
    if let Some(display) = frame.display() {
        let _ = distill_pager::views::status_line::render_status_line(
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = distill_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = distill_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use distill_pager::views::context_bar::fmt_tokens;
    ...
    if let Some(label) = agent.prompt_input_mode.prompt_info_override() {
        ...
        if let (Some(used), Some(total)) = (used, total)
            && total > 0
        {
            let pct = distill_token_estimation::usage_percentage(used, total);
            segs.push((
...
fn minimal_pending_hint(
    pending: &Option<distill_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> distill_pager::app::agent_view::AgentView {
    ...
    fn config_status_line_paints_and_records_the_script_size() {
        use std::sync::Arc;
        use distill_pager::views::status_line::{
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use distill_pager::app::agent::AgentState;
        use distill_pager::scrollback::RenderBlock;
        use distill_pager::scrollback::entry::ScrollbackEntry;
        use distill_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &distill_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use distill_pager::acp::tracker::TurnActivity;
        use distill_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        distill_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        let mut buf = Buffer::empty(area);
        ...
        distill_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            distill_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_style_bash_mode_shows_bang_prefix() {
        use distill_pager::app::agent_view::PromptInputMode;
        use distill_pager::appearance::AppearanceConfig;
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(distill_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &distill_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use distill_pager::app::actions::Action;
        use distill_pager::app::app_view::PendingAction;
        use distill_pager::input::key::KeyShortcut;
