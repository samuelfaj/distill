...
//!
//! It is printed via [`distill_ratatui_inline::Terminal::insert_before`], the same one-shot mechanism the commit pipeline uses.
...
pub fn maybe_commit_welcome(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    let border_color =
        distill_pager::render::color::blend_color(theme.bg_base, theme.gray_dim, 0.45)
