...
- Sessions save automatically. Resume the latest with `distill -c`, or pick
  one with `/resume` (`Ctrl+R`).
- Long session getting slow? `/compact` frees context; `/context` shows
  where it's going.
- Automate anything: `distill -p "summarize new TODOs" --output-format json`
  runs headless — great for scripts and CI.
- Stay current with `distill update`; see what changed with `/release-notes`.
