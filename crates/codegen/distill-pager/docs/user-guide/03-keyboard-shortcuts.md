...
Reference for key bindings in the Distill TUI. Bindings are built in and cannot currently be remapped.
...
- An unmodified middle click in Distill reads **PRIMARY**, the current mouse selection, only when `DISPLAY` is non-empty. Pure X11 can use its native reader fallback; XWayland requires `xclip` or `xsel` on `PATH` so Distill reads the X11 selection rather than Wayland PRIMARY. The press is handled once; the release does not paste again.
