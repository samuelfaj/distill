...
Agent mode runs Grok as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `distill -p` instead ([headless mode](14-headless-mode.md)).
...
```bash
distill agent --always-approve stdio
```
...
```bash
distill agent --always-approve --model grok-4.6 stdio
grok agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
distill agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
This is a server you run yourself — Grok's hosted cloud sandboxes do not run `distill agent serve`.
...
```bash
distill agent --always-approve headless --grok-ws-url wss://your-relay.example.com/ws
```
...
Beyond the base ACP protocol, Grok defines extension methods under the `x.ai/` prefix for provider-specific functionality. These cover:
...
The tables here show representative methods in each category. The `x.ai/*` set is provider-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
