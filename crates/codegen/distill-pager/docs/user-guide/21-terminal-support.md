...
Distill runs as a full-screen TUI. It relies on terminal support for color,
...
Grok cannot start, run `distill doctor` in your shell. Use `distill doctor --json`
...
`distill doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `distill doctor fix dcs-passthrough --yes`.
...
Distill detects these terminal emulators from environment variables:
...
Distill writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Distill runs inside tmux.
...
through `distill wrap`, for example `distill wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `distill wrap`, Grok shows the one-time tip
...
For repeated SSH use, Doctor offers `distill doctor fix ssh-wrap`. It also shows
...
> **Warning**: `distill wrap` is experimental and may not work in every setup.
...
`distill --no-alt-screen` to confirm inline mode works.
...
Run `distill doctor`, or run `/doctor` while voice mode is on. The **Voice** section
...
terminals, Terminal.app, Konsole, mlterm, and others). Distill therefore
