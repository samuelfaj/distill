...
Distill CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through the model provider.
...
- **Structurally separate** from provider-internal telemetry: its exporters carry
  only the headers you configure, never provider credentials.
- **Independent of provider data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams. Those
  settings govern provider-side retention; the external stream is governed solely
  by your own OTEL configuration.

### ZDR and this stream

`/privacy` and Zero Data Retention do **not** disable this stream. ZDR turns
off provider-side retention (product analytics, session-trace upload,
...
|---|---|---|
| `GROK_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `GROK_TELEMETRY_ENABLED`, which controls provider-internal product analytics — the two govern opposite-pointing data flows. |
...
Because Distill can force-disable this stream fleet-wide, the CLI holds emission
...
**The wait is bounded**, so a deployment that cannot reach Grok still exports:

- If no fleet policy can apply at all — `[features] remote_fetch = false`, or
  `[endpoints] cli_chat_proxy_base_url` points somewhere other than Grok — the
...
(`distill --debug`) for `external otel:` lines — they record whether the stream
...
## Metrics (meter scope `ai.distill`)
