...
Three files configure Distill, and they are written by different people.
...
Distill also reads these layers, later rows winning except where a requirements pin or the Managed column says otherwise.
...
Run `distill inspect` or `distill inspect --json` to see which files and values won.
...
| `endpoints.xai_api_base_url` | `string` | `pin` | `user` | Public Grok API base. Also GROK_XAI_API_BASE_URL. |
...
| `features.non_git_warning` | `boolean` | `yes` | `user` | Show a blocking warning when Distill starts outside a Git repository. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `tools.disable_zdr_incompatible_tools` | `boolean` | `yes` | `user` | Restrict tools that need Grok-hosted output under ZDR. Also GROK_DISABLE_ZDR_INCOMPATIBLE_TOOLS. |
...
| `ui.contextual_hints.ssh_wrap` | `boolean` | `yes` | `user` | Recommend `distill wrap` when SSH lacks a clipboard sink. |
...
Distill reads `/etc/grok/managed_config.toml` first, then `$GROK_HOME/managed_config.toml`, which the console keeps in sync. Values in the second replace values in the first.
...
| Situation | What Distill does |
| --- | --- |
| A developer sets a key you pinned | The pinned value applies. `distill inspect` lists the requirements file that contributed. |
...
| `requirements.toml` is missing or its signature does not verify | The pins do not apply, and Distill starts without them. Set `fail_closed = true` to refuse to start instead. |
...
Run `distill inspect` on the developer's machine. It lists every config file that contributed, including requirements and managed layers, so a policy that is not applying is visible in one command.
