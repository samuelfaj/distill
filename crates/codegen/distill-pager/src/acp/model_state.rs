...
impl ModelState {
    ...
    pub fn resolve_by_name_or_id(&self, query: &str) -> Option<acp::ModelId> {
        ...
        // Configured providers expose their catalog key through ACP, but the
        // routing id is often the value users know and paste into a tier field.
        // Resolve that id back to the visible entry so OpenRouter works in the
        // same picker as every other configured model.
        if query.contains('/')
            && let Some(key) = distill_shell::agent::config::configured_model_key(query)
        {
            let id = acp::ModelId::new(key);
        ...
        state
            .map(|state| {
                ...
                Self {
                    ...
                    effort_auto: distill_shell::jev::effort_auto_cached(),
