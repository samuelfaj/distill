...
#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use distill_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use distill_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
    ...
    fn apply_retry_state_pool_402_sets_credit_limit_blocked() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                error_type: "api".into(),
                message:
                    "API error (status 402 Payment Required): Distill usage balance exhausted"
                        .into(),
            },
            &mut session,
            &mut scrollback, false);
    ...
    fn compaction_lifecycle_refreshes_context_bar_from_trigger_counts() {
        ...
        let cancelled = XaiSessionUpdate::AutoCompactCancelled {
            reason: distill_shell::extensions::notification::AutoCompactCancelReason::UserCancelled,
        };
    ...
    fn child_compact_started_refreshes_context_used() {
        ...
        child_view.context_state = Some(distill_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .insert_test_child(child_sid.into(), Box::new(child_view));
    ...
    fn summary_generated_ext(
        ...
        let meta = if title_is_manual {
            Some(distill_shell::extensions::notification::title_is_manual_meta())
        } else {
            None
        };
    ...
    fn auto_title_notification_strips_controls_and_caps() {
        use distill_shell::session::persistence::MAX_TITLE_SCALARS;
        let mut app = make_app_with_agent("sess-1");
        let dirty = format!(
    ...
    fn hooks_changed_ext(
        session_id: &str,
        hooks: Vec<distill_hooks_plugins_types::HookInfo>,
    ) -> acp::ExtNotification {
        let notif = SessionNotification {
            session_id: acp::SessionId::new(session_id),
    ...
    fn push_hook(name: &str, source_dir: &str) -> distill_hooks_plugins_types::HookInfo {
