...
#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn late_unique_subagent_lifecycle_event_is_not_dropped() {
        ...
        // A persisted active goal update can arrive ahead of a lower-ID spawn; it advances the Distill highwater without adding a scrollback block
        assert!(handle(
            notification(
                serde_json::json!({
    ...
    mod eviction_and_rebuild {
        ...
        fn a_resumed_child_keeps_its_first_historical_prompt_and_paints_the_new_task_once() {
            ...
            crate::app::subagent::set_replay_distill_home_for_tests(Some(
    ...
    fn a_malformed_session_notification_is_ignored() {
        ...
        let msg = AcpClientMessage::ExtNotification(distill_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
    ...
    fn a_notification_reaches_its_target_agent_even_when_another_is_active() {
        // AutoCompactCompleted on the Distill ext path resets the context bar numerator via refresh_context_used
    ...
    fn spawn_labels_are_recorded_overwritten_on_wake_and_shared_with_child_trackers() {
        ...
        use distill_tools::implementations::distill::send_subagent_message::{
