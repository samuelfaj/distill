...
fn slash_gate_resyncs_when_critical_expires_between_pushes() {
    ...
    app.active_announcements = vec![distill_announcements::RemoteAnnouncement {
...
fn apply_auth_meta_disables_billing_surface_for_team_users() {
    let mut app = test_app();
    assert!(app.usage_visible);
    let meta = distill_login::AuthMeta {
...
fn apply_auth_meta_enables_billing_surface_for_personal_users() {
    let mut app = test_app();
    app.usage_visible = false;
    let meta = distill_login::AuthMeta::default();
    app.apply_auth_meta(&meta);
    assert!(app.usage_visible);
}
#[test]
...
fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
    ...
    app.apply_auth_meta(&distill_login::AuthMeta {
...
fn apply_auth_meta_clears_gate_on_subscription() {
    ...
    app.gate = Some(distill_login::GateInfo {
        message: "Subscribe to use Distill".into(),
        url: Some("https://grok.com/supergrok?referrer=Distill".into()),
        label: None,
    });
...
fn apply_auth_meta_gate_unchanged_when_still_gated() {
    let mut app = test_app();
    let gate = distill_login::GateInfo {
...
fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
    ...
    app.apply_foreign_resume_detection(
        launch_token,
        &canonical_cwd,
        Some(distill_foreign_sessions::RecentForeignSession {
...
fn menu_action_indices_match_the_shipped_menu() {
    ...
    assert!(matches!(
        dispatch_menu_action(1, false, true, Default::default()),
...
/// Provider buttons start login, including on the access-gated welcome menu.
#[test]
fn menu_action_provider_rows_start_login() {
    ...
    for (index, provider) in [(3, LoginProvider::ChatGpt), (4, LoginProvider::OpenRouter)] {
        for (row, import, access) in [
            ...
            assert!(matches!(
                dispatch_menu_action(row, import, access, Default::default()),
                InputOutcome::Action(Action::LoginProvider(actual)) if actual == provider
            ));
        }
    }
    assert!(matches!(
...
fn menu_action_indices_shift_by_the_import_row() {
    assert!(matches!(
        dispatch_menu_action(0, true, true, Default::default()),
        InputOutcome::Action(Action::ImportClaudeSettings)
    ));
    assert!(matches!(
        dispatch_menu_action(1, true, true, Default::default()),
        InputOutcome::Action(Action::OpenNewWorktreeDialog)
    ));
    assert!(matches!(
        dispatch_menu_action(2, true, true, Default::default()),
        InputOutcome::Action(Action::FetchSessionList)
    ));
    assert!(matches!(
        dispatch_menu_action(3, true, true, Default::default()),
        InputOutcome::Action(Action::Login)
    ));
    assert!(matches!(
...
#[test]
fn welcome_provider_rows_logout_only_the_connected_accounts() {
    ...
    assert!(matches!(
        dispatch_menu_action(2, false, true, state),
        InputOutcome::Action(Action::Logout)
    ));
    assert!(matches!(
        dispatch_menu_action(3, false, true, state),
        InputOutcome::Action(Action::LoginProvider(LoginProvider::ChatGpt))
    ));
    assert!(matches!(
    ...
    assert!(matches!(
        dispatch_menu_action(3, false, true, state),
        InputOutcome::Action(Action::LogoutProvider(LoginProvider::ChatGpt))
    ));
    let mut app = test_app();
    app.provider_auth = Some(state);
...
fn dashboard_picker_esc_after_search_click_restores_the_selection() {
    ...
    let snapshot = distill_dashboard_store::WorkspaceSnapshot {
...
fn overlay_left_arrow_file_search_open_does_not_exit() {
    let (mut app, id) = neutral_overlay_app();
    {
        ...
        agent.prompt.file_search.set_test_state(
            ctx,
            vec![distill_workspace::file_system::FuzzyMatchResult {
...
fn install_question_overlay(
    ...
    use distill_tools::implementations::distill::ask_user_question::{Question, QuestionOption};
