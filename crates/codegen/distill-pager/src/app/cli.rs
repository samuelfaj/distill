...
pub enum Command {
    /// Run Distill without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Distill discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting Distill
...
Examples:
  distill wrap docker exec -it my-container bash
  distill wrap kubectl exec -it my-pod -- bash
...
")]
    ...
    Update {
        ...
        enterprise: bool,
        /// Internal: what spawned this `distill update` (`user_command`, `auto_background`, `leader_converge`). Hidden.
        #[arg(long, hide = true)]
...
/// Targets a running leader process by PID (used by `distill leader` / `distill workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `distill leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
pub struct AgentArgs {
    ...
    /// Override the public Grok API base URL.
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "distill: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("distill: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
#[command(
    name = "distill",
    version = distill_version::full_version(),
    about = "Distill TUI",
...
pub struct PagerArgs {
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            distill_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `distill "fix the bug"` or `distill --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    pub(crate) fn local_resume_selection(
        &self,
    ) -> distill_shell::session::persistence::RecentSessionSelection {
        use distill_shell::session::unified_list::HeadlessPolicy;
        ...
        distill_shell::session::persistence::RecentSessionSelection::from_headless_policy(policy)
    }
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                distill_shell::session::persistence::resumed_session_sandbox_profile(Some(&id), cwd)
            }
            ResumeTarget::MostRecentForCwd => {
                distill_shell::session::persistence::resolve_recent_session_sandbox_profile(
                    cwd,
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<distill_sandbox::ProfileName>().ok()
                    != s.parse::<distill_sandbox::ProfileName>().ok() =>
            {
    ...
    /// This is the `distill "<prompt>"` launch form; the headless `-p`/`--single` path is handled separately.
...
mod tests {
    ...
    fn version_flags_parse_as_early_intent_without_exiting() {
        for flag in ["--version", "-v", "-V"] {
            let args = PagerArgs::try_parse_from(["distill", flag]).expect("version flag parses");
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["distill"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["distill", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["distill", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["distill", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json = PagerArgs::try_parse_from(["distill", "doctor", "--json"])
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["distill", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["distill", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["distill", "doctor", "all"],
            vec!["distill", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["distill", "doctor", "fix", "--yes"],
            vec!["distill", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["distill"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["distill", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["distill", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["distill", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["distill", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["distill", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["distill", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["distill", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["distill", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["distill", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["distill", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["distill", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["distill"]).expect("bare grok parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["distill", "leader", "list", "--json"])
        ...
        let info = PagerArgs::try_parse_from(["distill", "leader", "info", "--pid", "42"])
        ...
        let kill =
            PagerArgs::try_parse_from(["distill", "leader", "kill"]).expect("grok leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["distill", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["distill", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["distill", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args = PagerArgs::try_parse_from(["distill", "fix the bug"])
    ...
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args =
            PagerArgs::try_parse_from(["distill", "  spaced  "]).expect("padded prompt parses");
        assert_eq!(args.initial_prompt(), Some("spaced"));
        let blank = PagerArgs::try_parse_from(["distill", "   "]).expect("blank prompt parses");
    ...
    fn usage_command_parses_session_and_optional_turn() {
        let session_only = PagerArgs::try_parse_from(["distill", "usage", "sess-1"])
        ...
        let with_turn = PagerArgs::try_parse_from(["distill", "usage", "sess-1", "3"])
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["distill", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["distill"]).expect("bare grok parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["distill", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias = PagerArgs::try_parse_from(["distill", "--trust-folder"])
            .expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["distill", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias = PagerArgs::try_parse_from(["distill", "--effort", "high"])
    ...
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["distill", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep = PagerArgs::try_parse_from(["distill", "--reasoning-effort", "deep"])
            .expect("deep parses");
    ...
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["distill", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["distill", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["distill", "agent", "--effort", "max", "stdio"])
