...
pub(super) fn set_voice_stt_language_inner(app: &mut AppView, canonical: &str) {
    ...
    if language_changed && let Some(tx) = app.voice_cmd_tx.take() {
        app.voice_reset();
        let _ = tx.try_send(distill_voice::VoiceCommand::Shutdown);
    }
}
...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = distill_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == distill_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = distill_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
pub(super) fn set_follow_up_behavior_inner(
    ...
    distill_shell::util::config::set_follow_up_steer_cache(new.is_steer());
}
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut distill_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = distill_shell::util::config::resolve_contextual_hints(
...
///
/// The value is a `[model.<id>]` entry id on any provider, validated against
/// the catalog at the command boundary. Whether it may also share the session's
/// conversation (same provider, backend and credential) is decided in the shell
/// where the tier is resolved, and reported by `/tiers`; this only writes.
pub(in crate::app::dispatch) fn set_tier_light(
    ...
    if !id.is_empty() {
        let reasoning = match app.active_view {
            ActiveView::Agent(aid) => app
                .agents
                .get(&aid)
                .and_then(|agent| agent.session.models.current_model_id_str()),
            ActiveView::Welcome => app.models.current_model_id_str(),
            _ => None,
        };
        let Some(reasoning) = reasoning else {
            app.show_toast("Choose a reasoning model before setting a worker model.");
            return vec![];
        };
        if let Err(reason) = distill_shell::jev::validate_light_tier_candidate(reasoning, &id) {
            app.show_toast(&format!("Worker model not saved: {reason}"));
            return vec![];
        }
    }
...
/// Resets the persisted override to the built-in baseline (`distill_shell::models::default_model()`).
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = distill_shell::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();
...
pub(in crate::app::dispatch) fn set_cheap_model(
    ...
    let prev = distill_shell::jev::local_config_cached()
        .model
        .clone()
        .unwrap_or_default();
    if prev == entry
        && distill_shell::jev::local_config_cached()
            ...
            == effort.map(|e| e.to_string()).as_deref().unwrap_or("auto")
    {
        app.show_toast(&format!("\u{2713} Utility model: already {entry}"));
        return vec![];
    }
    let shown = if entry.is_empty() {
        "(cleared)"
    } else {
        &entry
    };
...
pub(in crate::app::dispatch) fn set_tier_editor(
    ...
    let Some(reasoning_id) = models.resolve_by_name_or_id(&reasoning) else {
    ...
    if !worker.is_empty()
        && let Err(reason) =
            distill_shell::jev::validate_light_tier_candidate(reasoning_id.0.as_ref(), &worker)
    {
        app.show_toast(&format!("Worker model not saved: {reason}"));
        return vec![];
    }
    if !utility.is_empty()
        && !(crate::slash::commands::cheap_model::standalone_chain(&utility)
            || crate::slash::commands::provider_status::openrouter_entries()
                .iter()
                .any(|(key, _)| key == &utility))
    {
        app.show_toast(
            "Utility model must be an OpenRouter entry or a comma-separated model chain.",
        );
        return vec![];
    }

    let mut effects = set_default_model(app, reasoning_id);
    ...
    match app.active_view {
        ...
        ActiveView::Welcome => app.welcome_doc_viewer = None,
        _ => {}
    }
    effects
}
