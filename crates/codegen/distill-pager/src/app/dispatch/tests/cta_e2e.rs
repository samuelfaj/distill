...
fn plugin_cta_catalog_loaded_sanitizes_components_at_ingestion() {
    ...
    let response = distill_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![distill_hooks_plugins_types::MarketplaceScanResult {
            source_name: distill_plugin_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: distill_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
            plugins: vec![entry],
            error: None,
        }],
    };
...
fn plugin_cta_catalog_keeps_official_not_installed_only() {
    ...
    let response = distill_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![
            ...
            distill_hooks_plugins_types::MarketplaceScanResult {
                ...
                source_kind: "git".into(),
                source_url_or_path: "git@github.com:example/plugin-marketplace.git".into(),
    ...
    assert_eq!(
        ...
        Some(distill_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL),
...
fn plugin_cta_default_prefers_url_verified_official_over_impostor() {
    ...
    // A first-listed source that merely calls itself "Grok Official" must not become the install root
...
fn plugin_cta_marketplace_override_selects_named_source() {
    ...
    let response = distill_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![distill_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_kind: "path".into(),
            source_url_or_path: "/srv/distill-marketplace".into(),
            plugins: vec![
    ...
    let response = distill_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![
            ...
            distill_hooks_plugins_types::MarketplaceScanResult {
                source_name: "Samuel Fajreldines Marketplace".into(),
                source_kind: "git".into(),
                source_url_or_path: "https://github.com/impostor/distill.git".into(),
                plugins: vec![cta_entry("impostor", "not_installed")],
                error: None,
            },
        ],
...
fn plugin_cta_marketplace_override_install_targets_named_source() {
    ...
    let id = AgentId(0);
    {
        ...
        agent.prompt.set_text("try zzdistillcta now");
    }

    let mut entry = cta_entry("zzdistillcta", "not_installed");
    entry.keywords = vec!["zzdistillcta".into()];
    let response = distill_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![distill_hooks_plugins_types::MarketplaceScanResult {
            source_name: "Samuel Fajreldines Marketplace".into(),
            source_kind: "path".into(),
    ...
    assert!(matches!(
        ...
        CtaPhase::Matched { name, .. } if name == "zzdistillcta"
    ));
    ...
    match effect {
        Effect::InstallPluginFromCta {
            ...
            assert_eq!(source_url_or_path, "/srv/distill-marketplace");
            assert_eq!(plugin_relative_path, "plugins/zzdistillcta");
        }
...
fn plugin_cta_marketplace_override_naming_official_selects_it() {
    let mut app = test_app_with_agent();
    app.plugin_cta_marketplace = Some(distill_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string());
    let id = AgentId(0);
...
fn plugin_cta_catalog_err_preserves_cache() {
    ...
    let id = AgentId(0);
    {
        ...
        cta.source_url_or_path = Some(distill_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into());
    ...
    for phase in [
        ...
        let id = AgentId(0);
        {
            ...
            cta.source_url_or_path =
                Some(distill_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into());
            cta.candidates = vec![cta_entry("figma", "not_installed")];
            cta.debounce_generation = 1;
...
fn cta_install_done_non_success_sets_error_with_sanitized_message() {
    ...
    let Some(&(pattern, expected)) = distill_shell::sampling::error::SERVICE_NAME_REWRITES.first()
    else {
...
mod cta_e2e {
    ...
    fn figma_candidate() -> distill_hooks_plugins_types::MarketplacePluginEntry {
        let mut entry = cta_entry("figma", "not_installed");
        entry.keywords = vec!["figma".into()];
    ...
    fn dismissed_plugin_suppressed_on_next_recompute() {
        ...
        let dismissed = distill_shell::config::dismissed_plugin_ctas_in_file(&config_path);
        let matched =
        ...
        distill_shell::config::add_dismissed_plugin_cta_to_file("figma", &config_path)
        ...
        for (enabled, source_present) in [(false, true), (true, false)] {
            ...
            app.plugin_cta_enabled = enabled;
            {
                ...
                cta.source_url_or_path = source_present
                    .then(|| distill_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string());
                cta.candidates = vec![figma_candidate()];
                cta.debounce_generation = 1;
                cta.phase = CtaPhase::Matched {
                    plugin_relative_path: "plugins/figma".into(),
