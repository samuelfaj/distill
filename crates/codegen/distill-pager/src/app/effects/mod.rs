...
use distill_acp_lib::{AcpAgentTx, acp_send};
use distill_telemetry::startup::{self, StartupPhase};
use actions::{
...
use distill_shell::sampling::error::http_status_from_error;
use distill_shell::session::{ExtMethodResult, SessionInfoResponse};
...
pub(crate) async fn discover_mcp_servers(
    ...
    let servers = tokio::task::spawn_blocking(move || distill_shell::util::config::load_mcp_servers(
            &cwd,
            &distill_tools::types::compat::CompatConfig::default(),
        ))
        .await
...
async fn create_session_in_backend_rpc(
    ...
    let rpc_span = match startup::current_phase_span().as_ref() {
        Some(parent) => {
            distill_telemetry::region!(
            "startup.session_create.backend_rpc",
            distill_telemetry::region::Parent::Explicit(parent)
        )
        }
        None => {
            distill_telemetry::region!(
            "session.create.backend_rpc",
            distill_telemetry::region::Parent::Inherit
        )
        }
    };
...
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = distill_active_sessions::register(distill_active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::LogoutProvider(provider) => {
            ...
            tasks.spawn(async move {
                ...
                if result.is_ok() {
                    if let Err(error) = distill_shell::cli_models::fetch_model_state(&model_tx).await {
                        tracing::warn!(%error, "Could not refresh models after provider logout");
                    }
                }
        ...
        Effect::SwitchAccount { request_seq, method_id, use_oauth } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    if let Err(error) = send_logout(&tx).await {
        ...
        Effect::ScanForeignSessions { cwd, compat, distill_home, coordinator, seq } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            distill_foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &distill_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || distill_foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
        ...
        Effect::FetchDashboardSessions => {
            ...
            tasks
                .spawn(async move {
                    ...
                    "cwd": cwd.to_string_lossy(),
                    "limit": 30,
                    "headless": distill_shell::session::unified_list::HeadlessPolicy::Exclude
                        .as_wire_str(),
                });
                    let request = acp::ExtRequest::new(
                        "x.ai/session/list",
            ...
            tasks
                .spawn(async move {
                    match tokio::task::spawn_blocking(move || {
                            let store = distill_dashboard_store::WorkspaceStore::open(
                                &db_path,
                            )?;
                            let snapshot = store.snapshot()?;
                            Ok::<
                                _,
                                distill_dashboard_store::StoreError,
                            >((store, snapshot))
                        })
                        .await
                    {
                        ...
                        Ok(Err(error)) => {
                            let retryable = matches!(&error, distill_dashboard_store::StoreError::Busy { .. });
                                        ...
                                        if let Err(error) = store.insert_member(member) {
                                            failures
                                                .push(WorkspaceMutationFailure {
                                                    ...
                                                    retryable: matches!(
                                            &error,
                                            distill_dashboard_store::StoreError::Busy { .. }
                                        ),
                                                });
                                        }
                                    }
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            use distill_shell::agent::session_registry_client::SessionRegistryClient;
            use distill_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = distill_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = distill_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = distill_shell::credential_factory::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        distill_shell::session::restore::ProgressCallback,
                    > = {
                        use distill_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
                    ...
                    match restore_session_with_storage(
                            ...
                            &session_id,
                            &cwd_str,
                            distill_shell::session::restore::RestoreSessionOpts {
                                turn_override: None,
                                progress,
                                restore_code: true,
                            },
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            let info = distill_shell::session::info::Info {
                                id: acp::SessionId::new(session_id),
                                cwd,
                            };
                            let history_path = distill_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use distill_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id, source } => {
            ...
            tasks
                .spawn(async move {
                    let params = distill_shell::extensions::task::KillTaskRequest {
        ...
        Effect::SetEffortAuto {
            ...
            tasks
                .spawn(async move {
                    use distill_shell::sampling::types::REASONING_EFFORT_AUTO_META_KEY;
        ...
        Effect::SwitchModel {
            ...
            tasks
                .spawn(async move {
                    let meta = effort
                        .map(|eff| {
                            use distill_shell::sampling::types::{
                            ...
                            let mut meta = acp::Meta::new();
                            meta.insert(distill_shell::sampling::types::REASONING_EFFORT_AUTO_META_KEY.into(), serde_json::Value::Bool(true));
                            Some(meta)
                        });
            ...
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            distill_feedback::read_regular_capped(
                                    &path,
                                    distill_shell::session::MAX_FEEDBACK_IMAGE_BYTES,
                                )
                                .map_err(|error| error.to_string())
                        })
                        .await
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            distill_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            distill_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    distill_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = distill_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistPluginCtaDismissed { plugin_id } => {
            tasks
                .spawn(async move {
                    if let Err(e) = distill_shell::config::run_add_dismissed_plugin_cta(
                            plugin_id,
                        )
                        .await
                    {
        ...
        Effect::PersistConsentAnswer { account, notice_id, version, acked } => {
            tasks
                .spawn(async move {
                    match distill_shell::util::config::set_consent_answer(
                            account,
                            notice_id,
                            version,
                            acked,
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = distill_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                distill_hooks_plugins_types::HooksListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                distill_hooks_plugins_types::PluginsListResponse,
                            >(inner.clone())
        ...
        Effect::FetchMemoryList { agent_id, session_id } => {
            use distill_shell::extensions::memory::{
        ...
        Effect::HooksAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = distill_hooks_plugins_types::HooksActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                distill_hooks_plugins_types::ActionOutcome,
                            >(inner.clone())
        ...
        Effect::PluginsAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = distill_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                distill_hooks_plugins_types::MarketplaceListResponse,
                            >(inner.clone())
        ...
        Effect::CheckMarketplaceUpdates { agent_id, session_id } => {
            if distill_workspace::permission::resolution::managed_settings()
                .plugin_auto_update
                .is_disabled()
            {
            ...
            tasks
                .spawn(async move {
                    ...
                    for (name, old, new, source_url, rel_path) in outdated {
                        let action = distill_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path: source_url.clone(),
                            plugin_relative_path: rel_path.clone(),
                        };
                        let req_body = distill_hooks_plugins_types::MarketplaceActionRequest {
                            session_id: session_id.0.to_string(),
                            action,
                        };
            ...
            tasks
                .spawn(async move {
                    let plugin_name = plugin_relative_path
                        ...
                        .to_string();
                    let action = distill_hooks_plugins_types::MarketplaceAction::Install {
                        source_url_or_path,
                        plugin_relative_path,
                    };
                    let req_body = distill_hooks_plugins_types::MarketplaceActionRequest {
        ...
        Effect::ReloadPluginsForCta { agent_id, session_id, plugin_name } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = distill_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action: distill_hooks_plugins_types::PluginsAction::Reload,
                    };
                    let req = acp::ExtRequest::new(
                        "x.ai/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: distill_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use distill_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model, nonce } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = distill_shell::agent::auth_method::has_xai_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback {
            ...
            use distill_shell::session::ClientType;
            use distill_shell::session::acp_types::{
                        ...
                        serde_json::value::to_raw_value(
                            &FeedbackDraftSendRequest {
                                ...
                                edited_body: FeedbackDraftEditedBody {
                                    input: distill_feedback::FeedbackDraftInput {
                                        title: draft.title,
                        ...
                        .await
                    {
                        Err(_elapsed) => {
                            TaskResult::FeedbackComplete {
                                agent_id,
                                origin,
                                outcome: distill_shell::session::FeedbackOutcome::OutcomeUnknown,
                        ...
                        Ok(send) => {
                            match send {
                                Ok(response) => {
                                    match serde_json::from_str::<
                                        distill_shell::session::FeedbackResponse,
                                    >(response.0.get()) {
                                        Ok(response) => {
                                            TaskResult::FeedbackComplete {
                                                agent_id,
                                                ...
                                                outcome: response
                                                    .outcome
                                                    .unwrap_or(
                                                        distill_shell::session::FeedbackOutcome::Submitted,
                                                    ),
                            ...
                            distill_acp_lib::acp_channel_failure(&error),
                            Some(distill_acp_lib::AcpChannelFailure::RecvFailed)
                        ) => {
                                    TaskResult::FeedbackComplete {
                                        agent_id,
                                        origin,
                ...
                .spawn(async move {
                    match request {
                                    ...
                                    send.map_err(|error| sanitize_user_error(
                                        ...
                                        .and_then(|response| {
                                            ...
                                            struct DraftListResponse {
                                                drafts: Vec<distill_feedback::FeedbackDraft>,
                                            }
                                    ...
                                    send.map_err(|error| sanitize_user_error(
                                        ...
                                        .and_then(|response| {
                                            ...
                                            struct DraftGetResponse {
                                                draft: distill_feedback::FeedbackDraft,
                                            }
                        ...
                        crate::views::feedback_modal::FeedbackDraftRequest::Update(
                            ...
                            let raw_params = match serde_json::value::to_raw_value(
                                &distill_shell::session::FeedbackDraftUpdateRequest {
                                    session_id: session_id.0.to_string(),
                ...
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let mode = match pinned_mode {
                                Some(mode) => mode,
                                None => distill_shell::config::load_memory_mode()?,
                            };
                            let storage = distill_shell::session::memory::MemoryStorage::new_for_mode(
                                &cwd,
                                None,
                                mode,
                            );
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            distill_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionMetaFromDisk {
            ...
            tasks
                .spawn(async move {
                    ...
                    let path = distill_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    ...
                    let (title, last_turn_summary) = tokio::task::spawn_blocking(move || -> Option<
                        ...
                        > {
                            ...
                            let summary: distill_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent, nonce } => {
            ...
            tasks
                .spawn(async move {
                    use distill_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "x.ai/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !distill_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let distill_home = distill_shell::util::distill_home::distill_home();
                            let store = distill_login::read_auth_json(
                                    &distill_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = distill_login::GrokComConfig::default()
                                .auth_scope();
                            let auth = distill_login::lookup_auth(&store, &scope)?;
                            let proxy_base = std::env::var(
                                    "GROK_CLI_CHAT_PROXY_BASE_URL",
                                )
                                .unwrap_or_else(|_| {
                                    distill_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            distill_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<distill_shell::extensions::notification::PromptUsage, String> {
    let request = acp::ExtRequest::new(
        "x.ai/session/usage",
        serde_json::value::to_raw_value(
                &serde_json::json!({
    ...
    let parsed: distill_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
async fn lookup_session_title_in(
    ...
    use distill_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
    let info = distill_shell::session::info::Info {
        id: session_id.clone(),
        cwd: cwd.to_string(),
    };
...
fn session_info_fields(
    ...
    push(
        "Shell version",
        distill_version::display_version(distill_update::channel_label()),
        false,
    );
    ...
    let model_display = distill_shell::session::model_display_name(
    ...
    if let Some(profile) = distill_sandbox::profile_name() {
