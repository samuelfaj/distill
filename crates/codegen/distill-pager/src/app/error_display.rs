...
impl WireErrorType {
    ...
    pub(crate) fn parse(raw: Option<&str>) -> Self {
        ...
        match s {
            ...
            s if s == distill_shell::extensions::notification::CONTEXT_LENGTH_ERROR_TYPE => {
                Self::ContextLength
            }
            "encrypted_content_mismatch" => Self::EncryptedContentMismatch,
            s if s == distill_shell::extensions::notification::DISK_FULL_ERROR_TYPE => {
                Self::DiskFull
            }
            s => s
                .parse::<distill_shell::sampling::error::SamplingErrorKind>()
                .map(Into::into)
...
impl From<distill_shell::sampling::error::SamplingErrorKind> for WireErrorType {
    fn from(kind: distill_shell::sampling::error::SamplingErrorKind) -> Self {
        use distill_shell::sampling::error::SamplingErrorKind as K;
        match kind {
            K::Auth => Self::Auth,
            K::Http => Self::Http,
            K::Api => Self::Api,
...
fn truncation_recovered_from_untyped_raw(error_type: Option<WireErrorType>, raw: &str) -> bool {
    error_type.is_none()
        ...
        && raw.contains(distill_shell::sampling::error::MAX_TOKENS_TRUNCATION_MESSAGE)
}
