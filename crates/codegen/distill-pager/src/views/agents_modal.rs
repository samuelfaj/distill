...
use distill_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use distill_shell::agent::config::AgentSelectionConfig;
use distill_tools::implementations::skills::discovery::extract_first_paragraph;
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as the shell: `[agent]`, `GROK_AGENT`, model `agentType`, then `Distill`).
...
/// DistillConcise, DistillPlan, DistillPlanNoSubagents, DistillAskUser, Codex, Opencode, CursorExtended, DistillOrchestrator.
...
impl AgentsModalState {
    ...
    #[cfg(test)]
    fn search_viewport(&self, width: usize) -> distill_ratatui_textarea::SingleLineViewport {
...
pub fn build_agent_list(
    cwd: &Path,
    toggle: &HashMap<String, bool>,
    plugins: Option<&distill_agent::plugins::PluginRegistry>,
) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = distill_agent::discovery::discover(cwd);
    ...
    if let Some(registry) = plugins {
        for agent in distill_agent::discovery::plugin_agents(registry) {
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = distill_home.join("bundled").join("personas");
    ...
    let dirs = [
        ...
        (ConfigFileScope::User, distill_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match distill_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => distill_config::distill_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".grok").join("personas"),
    }
}
...
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let distill_home = distill_config::distill_home();
    let in_user = dunce::canonicalize(distill_home.join(subdir))
        .ok()
...
pub(crate) fn load_agent_selection_config() -> AgentSelectionConfig {
    distill_shell::config::load_effective_config()
        .ok()
        .and_then(|root| distill_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// Mirrors `MvpAgent::resolve_agent_definition` in distill-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    distill_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn set_default_agent(name: Option<&str>) -> Result<(), String> {
    let config_path = distill_config::distill_home().join(distill_config::USER_CONFIG_FILENAME);
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        distill_agent::config::PromptMode::Extend => "extend",
        distill_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use distill_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn plugin_registry_with_reviewer(plugin_root: &Path) -> distill_agent::plugins::PluginRegistry {
        use distill_agent::plugins::discovery::PluginId;
        use distill_agent::plugins::{
