...
mod tests {
    ...
    fn pending_chatgpt_click_openrouter_escape_cancels_original_dispatch_token() {
        ...
        crate::app::dispatch::dispatch(Action::OpenOnboarding, &mut app);
        let area = Rect::new(0, 0, 80, 24);
        let mut render_connect = |app: &mut crate::app::app_view::AppView| {
        ...
        render_connect(&mut app);
        let chatgpt_row = app
            .onboarding
            .as_ref()
            .and_then(|state| state.hit_rows.iter().find(|(_, index)| *index == 1))
            .map(|(rect, _)| *rect)
            .expect("ChatGPT row rendered");
        let chatgpt_click = Event::Mouse(crossterm::event::MouseEvent {
            kind: MouseEventKind::Down(MouseButton::Left),
            column: chatgpt_row.x,
            row: chatgpt_row.y,
        ...
        let _ = crate::app::dispatch::dispatch(login, &mut app);
        ...
        render_connect(&mut app);
        let openrouter_row = app
            .onboarding
            .as_ref()
            .and_then(|state| state.hit_rows.iter().find(|(_, index)| *index == 2))
            .map(|(rect, _)| *rect)
            .expect("OpenRouter row rendered");
        let openrouter_click = Event::Mouse(crossterm::event::MouseEvent {
            kind: MouseEventKind::Down(MouseButton::Left),
            column: openrouter_row.x,
            row: openrouter_row.y,
            modifiers: crossterm::event::KeyModifiers::NONE,
        });
        assert!(matches!(
        ...
        let effects = crate::app::dispatch::dispatch(cancel, &mut app);
