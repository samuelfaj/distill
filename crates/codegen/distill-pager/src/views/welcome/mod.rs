...
use crate::views::prompt_widget::{PromptFlag, PromptInfo, PromptModelTiers, PromptWidget};
...
pub struct WelcomeRenderResult {
    ...
    /// Hit-test rect for the prompt footer's `change` action.
    pub model_tier_change_rect: Option<Rect>,
...
pub(super) struct WelcomeLayout {
    pub(super) logo: Rect,
    /// The wordmark line under the cat: `Distill` and the version.
...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a distill_announcements::RemoteAnnouncement>,
...
impl WelcomeLayout {
    /// Whether the hero box (side-by-side logo and menu) is active.
    ...
    /// Place the menu beside the mascot when both columns fit; otherwise stack them.
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Distill** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Distill**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = distill_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            spans.extend(title_spans(WORDMARK, theme));
            spans.push(Span::styled(
                format!("{}{}", distill_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
        }
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub provider_auth: crate::app::actions::ProviderAuthState,
    /// Folder-trust state.
    ...
    pub announcement: Option<&'a distill_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a distill_foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [distill_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a distill_login::GateInfo>,
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            ...
            let info = PromptInfo {
                ...
                model_tiers: params.model_tiers,
            };
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Distill is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Distill may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &distill_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let menu_items: &[(&str, &str)] = if !p.has_access {
        ...
        // Provider rows: Distill routes to more than one backend, so signing in
        // to each one, and picking the models a call may run on, belong in the menu.
        // Each row connects or disconnects only its own provider.
        items.push((
            "",
    ...
    let (cursor_pos, post_flush_escapes, model_tier_change_rect) = if show_picker {
        (None, None, None)
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://grok.com/supergrok?referrer=Distill");
...
fn masked_auth_token_view(input: &str, cursor_byte: usize, width: usize) -> (String, usize) {
    ...
    let buffer = distill_ratatui_textarea::EditBuffer::from_parts(
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Distill"), "full badge: {full:?}");
        assert!(inline.contains("Distill"), "inline badge: {inline:?}");
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use distill_foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = distill_foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn hero_box_active_on_wide_tall_terminal() {
        ...
        assert!(
            layout.has_hero_box(),
            "hero box should be active at the minimum width"
        );
    ...
    fn places_every_row(layout: &WelcomeLayout, input: &WelcomeLayoutInput<'_>) -> bool {
        ...
        (layout.has_hero_box() || stacked_rows)
            ...
            && layout.version.height + layout.wordmark.height == 1
    }
    ...
    fn hero_box_moves_up_only_once_the_flex_gap_is_gone() {
        let area = Rect::new(0, 0, HERO_BOX_MIN_WIDTH, 49);
        let input = |prompt_height| WelcomeLayoutInput {
            content_area: area,
            menu_height: 4,
            prompt_height,
    ...
    fn hero_box_boundary_at_min_width() {
        ...
        assert!(
            !layout.has_hero_box(),
            "hero box should be inactive below the minimum width"
        );
    ...
    fn hero_box_inactive_on_short_terminal() {
        ...
        assert!(
            !layout.has_hero_box(),
            "hero box should be inactive on a short terminal"
        );
    }

    #[test]
    fn hero_box_inactive_when_warning_would_overflow() {
        let area = Rect::new(0, 0, HERO_BOX_MIN_WIDTH, 42);
    ...
    fn hero_box_does_not_overflow_with_tall_menu() {
        let area = Rect::new(0, 0, HERO_BOX_MIN_WIDTH, 43);
        let layout = WelcomeLayout::compute(WelcomeLayoutInput {
            content_area: area,
            menu_height: 31,
            ..Default::default()
    ...
    fn hero_box_height_accounts_for_borders_and_padding() {
        ...
        assert!(layout.has_hero_box());
        assert_eq!(layout.hero_box.height, logo::full_logo_line_count() + 4);
    }
    ...
    fn hero_box_announcement_clamped_when_tight() {
        let area = Rect::new(0, 0, HERO_BOX_MIN_WIDTH, 40);
        let a = long_ann();
    ...
    #[test]
    fn hero_box_centers_menu_beside_cat() {
        let area = Rect::new(0, 0, HERO_BOX_MIN_WIDTH, 50);
        let a = long_ann();
        ...
        assert!(no_info.has_hero_box());
        for layout in [no_info, with_info] {
            assert!(layout.hero_menu.x >= layout.hero_logo.right());
            assert!(layout.hero_menu.width >= MENU_MIN_WIDTH);
            let above = layout.hero_version.y - layout.hero_logo.y;
            let below = layout.hero_logo.bottom() - layout.hero_menu.bottom();
            assert!(above.abs_diff(below) <= 1);
            assert!(layout.hero_menu.bottom() < layout.prompt.y);
        }
    }
    ...
    fn long_ann() -> distill_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn draft_never_shrinks_the_hero_announcement() {
        let area = Rect::new(0, 0, HERO_BOX_MIN_WIDTH, 49);
        let ann = distill_announcements::RemoteAnnouncement {
        ...
        assert!(
            saw_stacked,
            "the capped draft must leave too little room for the wide layout"
        );
    }
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = distill_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
