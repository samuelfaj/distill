...
pub use distill_tool_types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// Grok extension: request price in USD ticks (1 USD = 1e10 ticks).
...
impl ReasoningEffort {
    pub fn to_responses_api(self) -> crate::rs::ReasoningEffort {
        match self {
            ...
            // Ultra uses maximum server reasoning plus a request-local
            // delegation policy, applied by CreateResponseWrapper::body.
...
/// Grok and several OpenRouter models take the effort string. The shape belongs
...
pub fn reasoning_budget_tokens(level: ReasoningEffort) -> u32 {
    match level {
        ...
        ReasoningEffort::Max | ReasoningEffort::Ultra => 8_192,
    }
}
...
/// Wrapper around `async_openai::types::responses::CreateResponse` that adds custom header fields for Grok request tracking.
...
pub struct CreateResponseWrapper {
    ...
    /// Original effort, including levels the typed SDK cannot represent.
    pub reasoning_effort: Option<ReasoningEffort>,
    ...
    /// Grok-specific tool definitions that can't be expressed via `async_openai`'s `rs::Tool` enum (e.g., `x_search`).
...
impl CreateResponseWrapper {
    ...
    /// Ultra is a client mode: maximum reasoning plus proactive delegation.
    /// The Responses endpoint rejects the literal effort `ultra`.
    pub fn body(&self) -> serde_json::Result<serde_json::Value> {
        let mut body = serde_json::to_value(&self.inner)?;
        if let Some(effort) = self.reasoning_effort {
            body["reasoning"]["effort"] = serde_json::to_value(effort.to_responses_api())?;
            if effort == ReasoningEffort::Ultra {
                if let Some(text) = body["input"].as_str() {
                    body["input"] = serde_json::json!([{"role":"user", "content":text}]);
                }
                if let Some(input) = body["input"].as_array_mut() {
                    input.insert(0, serde_json::json!({
                        "role": "developer",
                        "content": "Ultra effort is selected. Proactively delegate independent subtasks using the available subagent tools when that helps solve the user's task. Keep the main work moving, integrate and verify results, and avoid delegation overhead for simple tasks. If subagent tools are unavailable, complete the task yourself."
                    }));
                }
            }
        }
...
/// Wrapper around `MessagesRequest` that adds custom header fields for Grok request tracking, analogous to `CreateResponseWrapper`.
#[derive(Debug, Clone, Default)]
...
mod tests {
    ...
    #[test]
    fn ultra_uses_max_wire_effort_and_request_local_delegation() {
        let mut request = CreateResponseWrapper::new(crate::rs::CreateResponse {
            input: crate::rs::InputParam::Text("hello".to_owned()),
            ..Default::default()
        });
