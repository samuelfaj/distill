...
- **`distill clone` on Windows** no longer fails when the repository's git config contains backslashes or quotes.
...
- **Multiple concurrent `distill agent stdio`** processes against the same GROK_HOME no longer crash at startup.
...
- **Headless sessions** (`distill -p`) now support the `--worktree` flag to run in a separate git worktree.
...
- **`distill du`** and worktree commands now work on Windows when only USERPROFILE is set.
...
- New `distill du` command shows disk usage of ~/.grok including worktrees and sessions.
...
- **Non-interactive sessions** (`distill -p`) no longer fail when the agent asks for user input or plan approval.
...
- **MCP servers** can now be enabled or disabled directly from the CLI with `distill mcp enable <name>` and `distill mcp disable <name>`.
...
- **Invalid MCP server entries** in config.toml no longer prevent Grok from starting; problems are shown in `distill inspect`.
...
- **`distill doctor fix`** can now repair common tmux clipboard and passthrough problems.
...
- **`distill --resume`** can now resume a session by its title as well as by ID.
...
- You can now run `distill doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- **grok doctor fix ssh-wrap** can set up `distill wrap ssh` automatically for Bash, zsh, and fish.
...
- **/btw** now works inside `distill --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **SSH sessions** now show a one-time tip recommending `distill wrap ssh <host>` for clipboard and terminal restore.
...
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `distill login`.
...
- **`distill plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **`distill worktree ls`** now works as a short alias for `distill worktree list`.
...
- You can now add a local directory as a plugin marketplace source with `distill plugin marketplace add`.
...
- **--minimal** flag now shows in `distill --help`.
...
- **Tool result previews** no longer paint opaque panels in `distill --minimal`.
...
- **`distill update`** downloads have a longer timeout.
...
- **Fixed `distill agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `distill wrap`** to run any command with local clipboard support.
...
- **Fixed `distill agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or grok-desktop.
...
- **`distill plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **`distill dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`distill mcp add`** now accepts positional arguments (e.g. `distill mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`distill update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed `distill --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `distill update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`distill --debug`** now produces per-session log files under ~/.grok/debug/ even with a leader process.
...
- **`distill update`** now restarts any older running leader so all clients get the new binary.
...
- **API-key users** can now run `distill agent --leader` without forced interactive login or timeouts.
...
- **Official Grok plugin marketplace** now appears automatically in the Marketplace tab on first launch.
