...
distill -p "Explain this codebase"
...
- **Reference**
  - [Introspection (`distill inspect`)](#introspection)
...
- [Building with Distill](#building-with-grok) — headless API, ACP SDK integration

---

## Installation

Build Distill from this repository:
...
Rebuild from source to update. The source installers under
`crates/codegen/distill-pager/scripts/` can install the executable in
`~/.local/bin`. Distill does not use an upstream binary update service.
...
Credentials are stored in `~/.grok/auth.json` and persist across sessions. Tokens expire after 7 days; Distill will prompt you to re-authenticate when needed.
...
1. Distill runs your command via `sh -c "<command>"`
...
**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Distill reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with grok.com". In headless mode (`distill -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
Distill runs your binary on two different contracts, and `GROK_AUTH_EXPIRED` is how it tells them apart:

| | `GROK_AUTH_EXPIRED=1` | unset |
|---|---|---|
| **What it is** | A headless refresh over a credential Grok already holds — near-expiry rotation, or a token the server rejected | A sign-in: `distill login`, the sign-in screen, or the escalation after a headless run couldn't mint |
...
**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `distill login` before starting — on first launch Distill runs your binary on the real terminal (URL and progress on stderr), then opens the UI already signed in. You _can_ run `distill login` to explicitly hydrate `auth.json` ahead of time if you prefer. Mid-session `/login` still uses the in-TUI copy-link overlay.
...
```bash
distill --debug-file /tmp/grok-auth.log -p "hello"
...
`auth_provider_command` above replaces Grok's *session* auth: it mints the token sent to Grok's backend. If you instead want Grok models on normal Grok login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- Distill runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Interaction with other credentials:** a literal `api_key`/`env_key` on the model wins over its `auth_provider`. Provider-backed models are BYOK: your Grok session token is never sent to their endpoints, and a failing provider command fails the request rather than falling back to the session token.
...
If you've authenticated with `distill login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the Distill CLI sends internally:
...
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  ...
  -H "x-grok-model-override: Distill" \
  -d '{
    "model": "Distill",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.grok/auth.json` (set by `distill login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-grok-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-grok-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `Distill` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `Distill`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `distill login` to refresh.
...
distill --cwd ~/projects/my-app

# Start with an initial task
distill --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
distill --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
distill --always-approve --prompt "Format all files"
```
...
| `-m, --model <MODEL>`   | Model to use (e.g., `Distill`)               |
...
distill -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
distill -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
distill -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
distill -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
distill -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
distill -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
distill -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
distill -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
distill -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
distill --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
distill -p "What does this project do?"

# Use a specific model
distill -p "Optimize this function" -m Distill

# Get JSON output for parsing
distill -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
distill -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
distill -p "Remember: the secret number is 42" --output-format json
distill -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
distill -p "Continue where we left off" -c

# Run in a different directory
distill -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
distill -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
distill -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
distill -p "Generate a README" > README.md

# Parse JSON output with jq
distill -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
distill -p "Review changes for bugs and security issues." \
...
# Pre-commit hook
distill -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `Distill`)                           |
...
```bash
distill agent headless --grok-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`distill ssh`)

Use `distill ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
distill ssh user@host

# With SSH flags
distill ssh -t user@host
distill ssh -L 8080:localhost:8080 user@host

# With remote command
distill ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `distill ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.

This runs entirely locally.

---

## Building with Distill

Distill can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `distill -p` and parses JSON output.
...
class GrokChat:
    ...
    async def create(self, messages, model="Distill", stream=False):
...
Distill reads configuration from `~/.grok/config.toml`. If the file doesn't exist, Distill uses sensible defaults. You only need to specify values you want to override.
...
Distill can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Grok language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Distill can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Distill can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.
...
Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Distill reads these files and appends their contents to the system prompt.
...
```bash
distill --agent-profile ./my-agent.md
# or
...
[subagents.models]
explore = "Distill"              # route explore to a lighter model
```
...
**How it works:** When you override a built-in model, Distill starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
> `distill models` output). You also need a `[model.*]` entry so Grok knows
...
**Auth behavior:** When `models_base_url` is set, Distill uses API key auth (`Authorization: Bearer`) instead of session auth. `distill login` is not required — only the API key.
...
Distill can restrict what the agent process and its spawned commands can access on
...
Use `distill inspect` to see everything Grok discovers in the current directory:

```bash
distill inspect          # human-readable output
distill inspect --json   # machine-readable JSON
```
...
| Component         | Claude Code location                                 | How Distill uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
...
distill -p "Remember: X=42" -s my-session
distill -p "What is X?" -s my-session

# Resume existing session (errors if not found)
distill -p "Continue" -r my-session

# Continue most recent session in current directory
distill -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
distill -p "Hello" --output-format json | jq -r '.sessionId'
```
...
distill completions bash > ~/.local/share/bash-completion/completions/grok
```
...
distill completions bash > ~/.grok/completions/bash/grok.bash
```
...
distill completions zsh > ~/.zsh/completions/_grok
```
...
distill completions zsh > ~/.grok/completions/zsh/_grok
```
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `distill --debug` or `GROK_LOG_FILE` instead:
...
GROK_LOG_FILE=/tmp/grok-debug.log RUST_LOG="info,distill_login=debug" grok
```
