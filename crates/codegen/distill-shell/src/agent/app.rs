...
#[cfg(test)]
use distill_login::AuthMode;
use distill_login::{AuthManager, GrokAuth, GrokComConfig, run_auth_flow};
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(distill_otel::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = distill_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// Register the process-lifetime runtime for shared filesystem watchers ([`distill_fsnotify::shared`]).
...
fn register_fs_watch_runtime() {
    distill_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
#[tracing::instrument(level = "debug", skip_all)]
pub async fn run_stdio_agent(
    ...
    distill_file_utils::queue::cleanup_orphaned_uploads(
        &distill_home::distill_home(),
        distill_file_utils::queue::DEFAULT_MAX_AGE,
    );
...
pub async fn run_headless(
    ...
    let (mut auth, did_browser_flow) = if reauthenticate {
        ...
        if crate::agent::auth_method::has_xai_api_key_env()
            ...
            && distill_login::try_ensure_fresh_auth(ctx, auth_manager.proxy_base_url().to_string())
                .await
                .is_none()
        {
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow {
            eprintln!();
            eprintln!(
                "Open Distill: {} (press Enter to open in browser)",
                grok_code_url
            );
            eprintln!();
...
struct DeferredRelayArm {
    ...
    slot: Rc<std::cell::RefCell<Option<crate::agent::relay::RelayHandle>>>,
    grok_com_config: distill_login::GrokComConfig,
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: distill_version::VERSION.to_string(),
    })
    ...
    let auth: Option<GrokAuth> =
        distill_login::try_noninteractive_auth_no_mint(ctx, agent_config.endpoints.proxy_url())
            .await;
    ...
    local_set
        .run_until(async move {
            ...
            if session_pending {
                ...
                tokio::task::spawn_local(async move {
                    tokio::select! {
                        ...
                        minted = distill_login::mint_session_noninteractive(&mint_auth_manager)
                            => match minted {
                            Some(session) => info!(
            ...
            let initial_auth_key_hash = distill_config::user_distill_home()
                .map(|g| g.join("auth.json"))
                .and_then(|auth_path| distill_login::read_auth_json(&auth_path).ok())
                .and_then(|store| {
                    distill_login::lookup_auth(&store, &auth_scope)
...
mod tests {
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = distill_login::GrokComConfig {
    ...
    fn embedded_otel_gate_keeps_a_session_user_fail_closed() {
        ...
        use distill_telemetry::external::{
    ...
    async fn deferred_arm_connects_relay_when_auth_appears() {
        ...
        let (_demand_tx, demand_rx) = watch::channel(false);
        let slot = Rc::new(std::cell::RefCell::new(None));
        let grok_com_config = distill_login::GrokComConfig {
            grok_ws_url: format!("ws://{addr}"),
            grok_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
