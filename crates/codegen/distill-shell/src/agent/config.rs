...
use distill_agent::prompt::skills::SkillsConfig;
use distill_login::{AuthManager, GrokComConfig, OidcAuthConfig};
use distill_sampler::{AuthScheme, SamplerConfig};
...
use distill_tools::types::compat::{
...
pub const DEFAULT_AGENT_TYPE: &str = "distill-plan";
...
pub struct EndpointsConfig {
    ...
    /// Base URL for the public Grok API.
...
impl EndpointsConfig {
    ...
    /// Uses the proxy default (not the `xai_api_base_url` fallback) so telemetry reports to Grok even when inference is overridden.
...
pub use distill_config::env_bool;
pub(crate) use distill_env::env_string;
...
pub(crate) fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> distill_chat_state::CompactionMode {
    use distill_chat_state::CompactionMode;
...
pub(crate) fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> distill_chat_state::CompactionDetail {
    use distill_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(distill_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: distill_tools::types::compat::CompatCell,
...
pub(crate) use distill_telemetry::config::env_telemetry_mode;
pub use distill_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    pub(crate) fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        if let Some(home) = distill_dirs::home_dir() {
            paths.push(home.join(".claude").join("settings.json"));
        }
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                distill_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub(crate) fn to_discovery_config(&self) -> distill_agent::plugins::discovery::DiscoveryConfig {
        distill_agent::plugins::discovery::DiscoveryConfig {
...
pub struct JevLocalConfig {
    /// Fixed effort override; absent or `auto` lets the harness choose.
    ...
    /// [`distill_workspace::jev::catalog::routing::LOCAL_CAPABLE_FLOOR`].
...
pub struct Config {
    ...
    #[serde(skip)]
    pub auth_providers: IndexMap<String, distill_config_types::AuthProviderConfig>,
    #[serde(skip)]
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<distill_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]`: also read by `distill_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<distill_agent::config::AgentDefinition>,
    #[serde(skip)]
    ...
    pub subagents_limit_behavior:
        distill_tools::implementations::distill::task::admission::LimitBehavior,
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, distill_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, distill_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<distill_tools::implementations::distill::video_gen::ZdrVideoOutputS3Config>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<distill_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<distill_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub(crate) fn apply_to_definition(&self, def: &mut distill_agent::config::AgentDefinition) {
    ...
    pub(crate) fn apply_to_subagent_definition(
        &self,
        def: &mut distill_agent::config::AgentDefinition,
    ) {
...
pub use distill_agent::config::Effort;
pub use distill_agent::config::PermissionMode;
pub use distill_shared::ui_config::{ContextualHints, UiConfig};
...
/// Default `Distill` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `distill_agent::discovery::by_name_in_cwd()`.
    /// Examples: "Distill", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(distill_version::VERSION.to_string()),
            ...
            media_gen_batch_limits: distill_tools::media_gen_limits::MediaGenBatchLimits::default(),
...
fn is_non_serde_config_path(path: &str) -> bool {
    NON_SERDE_CONFIG_PATHS.contains(&path)
        || distill_workspace::permission::resolution::MANAGED_POLICY_CONFIG_KEYS.contains(&path)
...
fn parse_auth_providers(
    ...
    IndexMap<String, distill_config_types::AuthProviderConfig>,
    ...
    for (name, value) in table {
        ...
        match serde_ignored::deserialize::<_, _, distill_config_types::AuthProviderConfig>(
            value.clone(),
...
impl Config {
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.grok_com_config.oidc.is_none() && config.grok_com_config.oauth2.is_none() {
            config.grok_com_config.oauth2 = distill_login::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.media_gen_batch_limits = distill_tools::media_gen_limits::MediaGenBatchLimits {
    ...
    fn apply_env_overrides(&mut self) {
        ...
        self.grok_com_config.force_login_team_uuid = distill_login::resolve_force_login_team(
            force_login_team_from_requirements(),
            distill_login::force_login_team_from_env(),
    ...
    pub fn product_analytics_enabled(&self, auth: Option<&distill_login::GrokAuth>) -> bool {
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<distill_sampling_types::DoomLoopRecoveryPolicy> {
        use distill_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
    ...
    /// Platform age-expiry (`process_cwd_scan_available`: linux+macos) is enforced inside `distill_fast_worktree::maybe_auto_gc`, not here.
    pub(crate) fn resolve_worktree_auto_gc(&self) -> distill_fast_worktree::ResolvedWorktreeAutoGc {
    ...
    /// Gate first-run auto-registration of the official Grok marketplace source. Precedence: env `GROK_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings > default off.
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        use distill_tools::implementations::distill::IMAGE_GEN_TOOL_NAME;
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use distill_tools::implementations::distill::IMAGE_EDIT_TOOL_NAME;
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> distill_chat_state::CompactionMode {
    ...
    fn resolve_compaction_detail(&self) -> distill_chat_state::CompactionDetail {
    ...
    /// Resolve whether to use grok's default OAuth2 (Grok auth.x.ai).
    /// Enterprise OIDC (`oidc` in config.toml) always wins; this only gates the default Grok OAuth2 fallback when no enterprise OIDC is configured.
...
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::distill_home::distill_home().join("requirements.toml");
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        distill_config::load_merged_requirements().as_ref(),
...
fn telemetry_otel_file_config(
    t: &toml::Value,
) -> distill_telemetry::external::ExternalOtelFileConfig {
    distill_telemetry::external::ExternalOtelFileConfig {
...
pub fn resolve_external_otel_config(
    client: distill_telemetry::external::config::ExternalClientInfo,
) -> Option<distill_telemetry::external::ExternalOtelConfig> {
    let requirements = distill_config::load_merged_requirements();
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<distill_telemetry::external::ExternalOtelFileConfig> = effective_config
    ...
    let mut resolved = distill_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub(crate) fn apply_external_otel_remote_policy(
    ...
    let policy = distill_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        distill_telemetry::external::apply_remote_policy(policy);
    }
}
...
pub fn apply_remote_settings_side_effects(
    ...
    distill_config::signed_policy::apply_remote_managed_config_signature_verification(
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = distill_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    distill_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub(crate) fn resolve_model_list(
    ...
    for (key, model_override) in &cfg.config_models {
        ...
        let base = resolved.shift_remove(key).or_else(|| {
            // A configured ChatGPT alias inherits the account's capabilities;
            // explicit local settings are still applied below.
            codex_models
                .values()
                .find(|entry| {
                    model_override.model.as_deref() == Some(entry.info.model.as_str())
                        && model_override.base_url.as_deref().is_some_and(|url| {
                            url.trim_end_matches('/') == entry.info.base_url.trim_end_matches('/')
                        })
                })
                .cloned()
                .map(|mut entry| {
        ...
        {
            entry.auth_provider = Some(distill_login::AuthProviderRef::fail_closed(format!(
...
/// Resolve a configured model's catalog key from its key or routing id.
///
/// The pager receives the catalog key through ACP, while users often type the
/// provider's routing id directly (for example `qwen/model` on OpenRouter).
/// Keeping this lookup here makes both forms select the same configured entry.
pub fn configured_model_key(model_id: &str) -> Option<String> {
    let raw = crate::config::load_effective_config().ok()?;
    let cfg = Config::new_from_toml_cfg(&raw).ok()?;
    let models = resolve_model_list(&cfg, None);
    configured_model_key_in(&models, model_id)
}

pub(crate) fn configured_model_key_in(
    models: &IndexMap<String, ModelEntry>,
    model_id: &str,
) -> Option<String> {
    models
        .iter()
        .find(|(key, entry)| key.as_str() == model_id || entry.info.has_model_id(model_id))
        .map(|(key, _)| key.clone())
}
...
pub struct ModelEntryConfig {
    ...
    /// Without this set, Grok API models send args as one delta event, defeating the purpose of streaming.
...
impl ConfigModelOverride {
    pub(crate) fn apply(
        ...
        if let Some(ref name) = self.auth_provider {
            entry.auth_provider = Some(distill_login::AuthProviderRef::unresolved(name.clone()));
        }
...
pub struct ModelInfo {
    ...
    /// When true, this model uses concise mode (compact system prompt, concise tool output, concise user message prefix, reduced toolset).
    pub use_concise: bool,
    /// Always has a value; defaults to `"distill-plan"` when the server or user config doesn't specify one.
    #[serde(default = "default_agent_type")]
    pub agent_type: String,
...
pub struct ModelEntry {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub auth_provider: Option<distill_login::AuthProviderRef>,
...
impl ModelEntry {
    ...
    /// Non-empty `api_key`, then `env_key`, then the saved OpenRouter credential.
    ...
    pub(crate) fn own_credential(&self) -> Option<String> {
        first_own_credential(self.api_key.as_deref(), self.env_key.as_ref()).or_else(|| {
            // Browser sign-in applies only to OpenRouter; custom credentials keep precedence.
            if self.auth_provider.is_none()
                && crate::openrouter_auth::is_openrouter_url(&self.info.base_url)
                && self
                    .env_key
                    .as_ref()
                    .is_none_or(|keys| keys.names() == [crate::openrouter_auth::API_KEY_ENV])
            {
                crate::openrouter_auth::api_key().ok().flatten()
            } else {
                None
            }
        })
    ...
    pub(crate) fn effective_auth_provider(&self) -> Option<&distill_login::AuthProviderRef> {
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<distill_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when Grok starts outside a Git repository. Default: false. Used as the local fallback when the `non_git_warning` remote settings flag in `distill_settings` is absent.
...
pub(crate) struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: distill_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
pub(crate) fn resolve_credentials(
    ...
    } else if !info.base_url.is_empty()
        && crate::codex_auth::is_trusted_inference_base_url(&info.base_url)
    {
        // The Codex bearer resolver supplies this account's OAuth token. An
        // unrelated XAI_API_KEY must not override it.
        (
            None,
            info.base_url.clone(),
            distill_chat_state::AuthType::ApiKey,
        ...
        )
    {
        ...
        (Some(key), url, distill_chat_state::AuthType::ApiKey)
    } else {
...
/// `disable_api_key_auth` at the credential seam: swap a first-party Grok API key for the IdP session.
/// When no session is available the request fails and forces a login.
/// BYOK (non-Grok `base_url`) is untouched; no-op when the switch is off.
pub(crate) fn enforce_disable_api_key_auth(
    ...
    if disable_api_key_auth
        && creds.auth_type == distill_chat_state::AuthType::ApiKey
        && crate::util::is_xai_api_url(&creds.base_url)
    {
        creds.auth_type = distill_chat_state::AuthType::SessionToken;
...
pub use distill_telemetry::config::deployment_id_from_key;
...
pub(crate) fn resolve_model_auth_facts_and_provider(
    model_id: &str,
) -> (ModelAuthFacts, Option<distill_login::AuthProviderRef>) {
...
/// That avoids forcing `image_description_model` onto the agent endpoint, which 404s on BYOK / non-proxy routes for internal slugs like `Distill`.
...
pub(crate) fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: distill_chat_state::AuthType,
) -> distill_chat_state::AuthType {
    try_resolve_model_credentials(model_id, session_key)
        .map(|r| r.auth_type)
        .unwrap_or(fallback)
}
/// Selects Grok-only Responses extensions for trusted backend-search routes.
...
fn force_login_team_from_requirements() -> Option<distill_login::ForceLoginTeam> {
    distill_login::force_login_team_from_requirements_value(
