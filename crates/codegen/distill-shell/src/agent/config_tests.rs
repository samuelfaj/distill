...
fn main_cli_tools_override_preserves_profile_injection_policy() {
    ...
    let mut cases = vec![(AgentDefinition::default_distill(), true)];
...
fn auto_mode_config_parses_from_toml_and_json_equivalently() {
    use distill_workspace::permission::ClassifierPromptType;
    let toml_src = r#"
enabled = true
...
fn finalize_image_describe_sampler_some_stamps_session_fields() {
    ...
    assert_eq!(model, "Distill");
    assert_eq!(cfg.model, "Distill");
    assert_eq!(cfg.client_identifier.as_deref(), Some("cli"));
    assert_eq!(cfg.max_retries, Some(7));
}
#[test]
fn resolve_aux_model_honors_distill_override() {
    let endpoints = EndpointsConfig::default();
    ...
    let resolved =
        resolve_aux_model_sampling_config("Distill", &catalog, &endpoints, None, false, None, None)
...
async fn aux_model_with_auth_provider_never_reroutes() {
    let endpoints = EndpointsConfig::default();
    let provider = distill_login::AuthProviderRef::new(
        "aux-provider-test".into(),
        distill_login::AuthProviderConfig {
    ...
    assert!(
        ...
        .is_none(),
        "cold provider cache must not reroute the aux model through the Distill proxy"
    );
...
fn session_resolver_is_not_stamped_onto_third_party_samplers() {
    ...
    impl distill_sampler::BearerResolver for SessionResolver {
...
fn shell_environment_policy_known_keys_track_the_policy_struct() {
    let distill_tools::util::ShellEnvironmentPolicy {
        inherit: _,
        ignore_default_excludes: _,
        exclude: _,
        set: _,
        include_only: _,
    } = distill_tools::util::ShellEnvironmentPolicy::default();
...
fn parses_auth_provider_tables_and_model_reference() {
    ...
    assert_eq!(
        ...
        Some(&distill_login::AuthProviderConfig {
...
async fn static_key_shadows_defined_provider_through_pipeline() {
    ...
    assert_eq!(creds.auth_type, distill_chat_state::AuthType::ApiKey);
...
async fn resolve_credentials_serves_cached_provider_token() {
    use distill_chat_state::AuthType;
...
fn prefetched_entry_provider_config_comes_from_trusted_tables_only() {
    ...
    let smuggled: distill_login::AuthProviderRef =
    ...
    assert_eq!(
        provider.config,
        distill_login::AuthProviderConfig::default()
    );
...
fn sampling_config_uses_fallback_when_no_model_api_key() {
    ...
    let sampling_config = sampling_config_for_model(
        ...
        ResolvedCredentials {
            ...
            auth_type: distill_chat_state::AuthType::ApiKey,
...
fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
    ...
    assert_eq!(config.api_backend, ApiBackend::Messages);
    let client = distill_sampler::SamplingClient::new(config).expect("client should build");
...
fn user_override_parses_compaction_at_tokens_from_toml() {
    use distill_sampling_types::CompactionAtTokens;
    let dm = crate::models::default_model();
...
fn user_override_parses_compactions_remaining_from_toml() {
    use distill_sampling_types::CompactionsRemaining;
    let dm = crate::models::default_model();
    let raw_config: toml::Value = toml::from_str(&format!(
        r#"
            [model."{dm}"]
...
fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
    ...
    assert_eq!(
        ...
        CompactionMode::Segments(distill_chat_state::CompactionDetail::default())
    );
    assert_eq!(
...
#[test]
fn model_agent_type_defaults_to_distill() {
    let raw_config: toml::Value = toml::from_str(
        r#"
            [model.my-model]
            model = "my-model"
...
fn force_login_team_uuid_implies_api_key_auth_disabled() {
    use distill_login::{ForceLoginTeam, GrokComConfig};
...
#[test]
fn configured_model_key_accepts_an_openrouter_routing_id() {
    let (_, models) = resolve_models_from_toml(
        r#"
    ...
    assert_eq!(
        configured_model_key_in(&models, "qwen/qwen3.7-flash").as_deref(),
...
fn a_model_entry_carries_its_reasoning_shape_into_the_sampler_config() {
    ...
    assert_eq!(
        ...
        distill_sampling_types::ReasoningShape::MaxTokens
    );
    ...
    assert_eq!(
        ...
        distill_sampling_types::ReasoningShape::None
    );
}
#[test]
#[serial]
...
"#;
    ...
    let leaked: Vec<&String> = unused
        .iter()
        .filter(|key| {
            distill_workspace::permission::resolution::MANAGED_POLICY_CONFIG_KEYS
                .iter()
                .any(|policy_key| {
...
fn worktree_auto_gc_section_parses_from_toml() {
    unsafe { distill_fast_worktree::clear_auto_gc_env_for_test() };
    let raw: toml::Value = toml::from_str(
        r#"
            [worktree.auto_gc]
            enabled = true
    ...
    assert_eq!(
        p.max_age_by_kind
            .get(&distill_fast_worktree::WorktreeKind::Subagent),
        Some(&Some(3600))
    );
    assert_eq!(
        p.max_age_by_kind
            .get(&distill_fast_worktree::WorktreeKind::Manual),
        Some(&None)
    );
}
...
[goal]
enabled = true
planner_model = { model = "Distill", agent_type = "distill-plan" }
...
"#;
    ...
    let cfg = Config::new_from_toml_cfg(&raw).unwrap();
    assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "Distill");
    assert_eq!(
...
planner_model = { agent_type = "distill-plan" }
"#;
    let raw: toml::Value = toml::from_str(toml_str).unwrap();
...
"#,
    ...
    let distill = crate::util::config::GoalRoleModel {
        model: "Distill".into(),
        agent_type: "distill-plan".into(),
    };
    ...
    assert_eq!(
        planner.value,
        GoalRoleModelChoice::Explicit(distill.clone())
    );
    ...
    assert_eq!(
        ...
        vec![
            GoalRoleModelChoice::Explicit(distill),
...
fn ext_client() -> distill_telemetry::external::config::ExternalClientInfo {
    distill_telemetry::external::config::ExternalClientInfo::default()
}
#[test]
...
fn external_otel_pin_protocol_hides_unlisted_file_siblings() {
    ...
    assert_eq!(
        cfg.logs_transport,
        distill_telemetry::external::config::OtlpTransport::HttpProtobuf,
...
fn external_otel_pin_exporter_beats_none() {
    ...
    assert_eq!(
        cfg.logs_exporter,
        distill_telemetry::external::config::ExporterSelection::Otlp
    );
}
#[test]
...
"#,
    ...
    distill_config::apply_version_overrides(&mut value, &v).unwrap();
    let cfg = Config::new_from_toml_cfg(&value).unwrap();
    assert_eq!(cfg.models.default.as_deref(), Some("grok-4.5"));
}
/// Reproduce the enterprise managed config bug: [model.Distill] sets context_window=500k for model="grok-4.5". [models].default="grok-4.5" still resolves to the bare prefetched entry (256k).
/// Layer 3 only overrides key "Distill", not key "grok-4.5". After the Layer 4 slug propagation fix, both keys should have 500k.
#[test]
fn slug_propagation_enterprise_managed_config_key_mismatch() {
    ...
    let resolved = resolve_model_list(&cfg, Some(prefetched));
    let by_key = resolved.get("Distill").expect("Distill key must exist");
    ...
    assert_eq!(
        ...
        "BUG: prefetched 'grok-4.5' should inherit 500k from \
         sibling 'Distill' (same model slug), not stay at {default_cw}"
    );
}
...
fn remote_settings_disarm_managed_config_signatures() {
    ...
    assert!(distill_config::signed_policy::verification_active());
...
fn remote_settings_disarm_requires_prod_proxy_when_keys_embedded() {
    ...
    apply_remote_settings_side_effects(Some(&settings), prod);
    assert!(
        !distill_config::signed_policy::verification_active(),
...
async fn process_key_from_model_env_key() {
    use distill_login::{AuthManager, GrokComConfig, shared_api_key_provider};
    use std::sync::Arc;
    ...
    let dm = distill_models::default_model();
    let cfg = Config::new_from_toml_cfg(
...
#[test]
#[serial]
fn openrouter_login_credentials_are_scoped_to_its_provider() {
    let _key = EnvGuard::set("OPENROUTER_API_KEY", "test-openrouter-key");
    let mut entry = test_model_entry(
        "openrouter-model",
    ...
    assert_eq!(
        resolve_credentials(&entry, Some("grok-session"))
            .api_key
            .as_deref(),
        Some("test-openrouter-key")
    );
    entry.api_key = Some("explicit-key".to_owned());
    assert_eq!(entry.own_credential().as_deref(), Some("explicit-key"));
    entry.api_key = None;
    entry.info.base_url = "https://openrouter.ai.example.com/api/v1".to_owned();
    assert!(entry.own_credential().is_none());
    entry.info.base_url = "https://chatgpt.com/backend-api/codex".to_owned();
    assert!(entry.own_credential().is_none());
}
