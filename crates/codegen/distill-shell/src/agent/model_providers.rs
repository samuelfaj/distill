...
pub struct ModelProviderConfig {
    ...
    pub auth: Option<distill_config_types::AuthProviderConfig>,
...
pub(crate) fn auth_config_issues(
    config: &distill_config_types::AuthProviderConfig,
    ...
    let skew = distill_login::PROVIDER_TOKEN_EXPIRY_SKEW_SECS;
    ...
    if let Some(timeout) = config.timeout_secs
        && !(1..=distill_login::PROVIDER_TIMEOUT_CEILING_SECS).contains(&timeout)
    {
        let ceiling = distill_login::PROVIDER_TIMEOUT_CEILING_SECS;
        issues.push((
            "timeout_secs",
            ConfigWarningKind::InvalidValue,
            if timeout == 0 {
