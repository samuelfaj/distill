...
use distill_login::PreferredAuthMethod;
use crate::upload::trace::PromptMetadataParams;
use distill_tools::implementations::distill::task::backend::SubagentBackend;
use distill_tty_utils::ProcessScope;
struct SessionConfigInputs {
    model_id: acp::ModelId,
    effort_options: Vec<ReasoningEffortOption>,
    current_effort: Option<distill_sampling_types::ReasoningEffort>,
}
...
type PolledFields = (
    Option<Vec<distill_announcements::RemoteAnnouncement>>,
    Vec<distill_config_types::RemoteRequestEncoding>,
);
...
impl MvpAgent {
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<distill_login::GrokAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    ) -> (bool, distill_agent::plugins::discovery::DiscoveryConfig) {
    ...
    ) -> Option<std::sync::Arc<distill_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        distill_login::credential_provider::sync_external_otel_identity();
        let cfg = self.cfg.borrow();
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_xai_auth() || a.auth_mode == distill_login::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            distill_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                distill_version::VERSION.to_owned(),
                subscription_tier,
    ...
    pub(crate) fn current_auth(&self) -> Option<distill_login::GrokAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &distill_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn arm_local_workspace_watcher(
        ...
        let auth_path = distill_workspace::hub_auth::default_auth_path().ok();
    ...
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<distill_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    distill_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    distill_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match distill_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => distill_workspace::WorkspaceOps::local(handle),
            Err(e) => {
                tracing::error!(
                    error = %e,
    ...
    pub(crate) fn auth_type(&self) -> distill_chat_state::AuthType {
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let Some(method_id) = self.cached_token_fallthrough_method_id() else {
            ...
            let msg = match preferred {
                Some(distill_login::PreferredAuthMethod::ApiKey) => {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &distill_login::GrokAuth) {
    ...
    pub(super) fn spawn_post_auth_settings(&self, auth: distill_login::GrokAuth) {
        let agent_ref = LocalRef::new(self);
        let _spawned = self
            .spawn_coalesced_settings_task(
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if prefers_oidc && !model.has_own_credentials()
            && credentials.auth_type == distill_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = distill_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == distill_chat_state::AuthType::ApiKey
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> distill_tools::implementations::distill::image_gen::ImageGenConfig {
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| distill_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("xai-Distill/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use distill_tools::implementations::distill::app_builder::AppBuilderDeployerConfig;
        AppBuilderDeployerConfig::Disabled
    }
    /// Video tools call the Distill API directly.
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> distill_tools::implementations::distill::video_gen::VideoGenConfig {
    ...
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> distill_tools::implementations::distill::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        let instance = Self {
            ...
            plugin_registry_handle: distill_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: distill_tools::implementations::distill::monitor::types::MonitorEventBuffer::default(),
        ...
        distill_login::credential_provider::wire_otel_auth_manager(
            instance.auth_manager.clone(),
        );
        if let Some(ref dk) = instance.cfg.borrow().endpoints.deployment_key {
            distill_login::credential_provider::wire_otel_deployment_key(dk.clone());
        }
        instance
    }
    ...
    pub(crate) async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
        source: distill_tools::types::KillSource,
    ) -> Result<distill_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub(crate) async fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> distill_tools::implementations::distill::task::types::SubagentCancelOutcome {
        distill_tools::implementations::distill::task::backend::ChannelBackend::new(
    ...
    pub(crate) async fn list_running_subagents(
        ...
        distill_tools::implementations::distill::task::types::SubagentInspection,
    > {
        let backend = distill_tools::implementations::distill::task::backend::ChannelBackend::new(
            self.subagent_event_tx.event_sender().0,
        );
    ...
    pub(crate) async fn query_subagent(
        ...
        distill_tools::implementations::distill::task::types::SubagentSnapshot,
    > {
        distill_tools::implementations::distill::task::backend::ChannelBackend::new(
                self.subagent_event_tx.event_sender().0,
            )
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<distill_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub(crate) async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<distill_hooks_plugins_types::HooksListResponse> {
    ...
    pub(crate) async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: distill_hooks_plugins_types::HooksAction,
    ) -> Option<distill_hooks_plugins_types::ActionOutcome> {
        if matches!(action, distill_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&distill_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub(crate) async fn execute_plugins_action(
        &self,
        session_id: &acp::SessionId,
        action: distill_hooks_plugins_types::PluginsAction,
    ) -> Option<distill_hooks_plugins_types::ActionOutcome> {
        let is_reload = matches!(action, distill_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(|o| &o.status),
            Some(distill_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub(super) fn diagnostic_upload_config(
        &self,
    ) -> Option<distill_login::DiagnosticUploader> {
        self.sync_collection_config_gate();
        let cfg = self.cfg.borrow();
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<distill_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<distill_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, distill_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata::new(PromptMetadataParams {
                ...
                shell_version: Some(distill_version::VERSION.to_string()),
                sandbox: local_sandbox_telemetry(),
                ..Default::default()
            });
            let capture = distill_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                distill_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let distill_home = crate::util::distill_home::distill_home();
    ...
    /// Strict-harness classification is structural; see [`distill_agent::config::is_strict_harness_agent_type`]. Harness inheritance for a profile that pins its own model is applied by the caller via [`inherited_harness_template`], not here.
    pub fn resolve_agent_definition(
        cwd: &std::path::Path,
        agent_profile_path: Option<&std::path::Path>,
        agent_config: &config::AgentSelectionConfig,
        acp_agent_profile: Option<distill_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> distill_agent::AgentDefinition {
        use distill_agent::AgentDefinition;
        ...
        let model_requires_strict_harness = model_agent_type
            .is_some_and(distill_agent::config::is_strict_harness_agent_type);
        if !grok_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = distill_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
                agent_name = %def.name,
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = distill_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
        ...
        let resolved = match agent_name.as_deref() {
            ...
            Some("distill-concise") | Some("grok_build_concise") => {
                AgentDefinition::distill_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::distill_plan()
                    }
                }
            }
            Some(name) => {
                distill_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::distill_plan)
            }
            None => AgentDefinition::distill_plan(),
        };
        if !grok_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = distill_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
                model_agent_type = %required,
    ...
    pub(super) fn resolve_status_line_capability(
        ...
        Self::resolve_bool_capability(
            meta,
            init,
            distill_status_line::CLIENT_STATUS_LINE_META,
            distill_status_line::STATUS_LINE_CAPABILITY,
            false,
        )
    }
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let envrc = envrc
            .unwrap_or_else(|| distill_workspace::envrc::spawn_envrc_load(
        ...
        let fs: Arc<dyn distill_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (distill_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = distill_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = distill_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
                ...
                tokio::spawn(
                    distill_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = distill_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        ...
        // A local or independently authenticated model needs no Grok auth
        // method. Its own credentials are resolved when inference is sent.
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            distill_agent::config::ModelOverride::Override(id) => {
            ...
            distill_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = distill_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = distill_tools::util::distill_home::distill_home()
                    .join("lsp.json");
        ...
        let (init, session_thread) = {
            ...
            let credentials = distill_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                distill_sampler::SharedAttributionCallback,
            > = Some(
                distill_login::attribution::ShellAttribution::new(
                    self.auth_manager.clone(),
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = distill_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                        &format!(
                        "{}{}",
                        distill_hooks::config::AGENT_HOOK_PREFIX,
                    ...
                    let git_root = {
                        ...
                        distill_workspace::session::git::find_git_root_from_path(cwd)
                            .ok()
                    };
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    distill_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
                    ...
                    Some(
                        Arc::new(
                            distill_login::manager::SharedAuthKeyProvider(
                                self.auth_manager.clone(),
                            ),
                        ),
                    ),
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        distill_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == distill_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        distill_hunk_tracker::LocAggregate::LinesReverted {
