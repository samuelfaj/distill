...
use distill_telemetry::id::{agent_id, agent_instance_id};
use distill_telemetry::session_ctx::log_event;
use distill_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use distill_workspace::permission::{ClientType, PermissionEvent};
...
use distill_workspace::session::git::GitDiscoveryResult;
use distill_hunk_tracker::HunkTrackerActor;
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&distill_login::GrokAuth>,
) -> Option<String> {
    if let Some(t) = display.filter(|s| !s.trim().is_empty()) {
        return Some(t);
    }
    let auth = auth?;
    if auth.auth_mode == distill_login::AuthMode::ApiKey {
...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub envrc: Option<distill_workspace::envrc::EnvrcLoad>,
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a distill_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[distill_announcements::RemoteAnnouncement]>,
    last_emitted: &[distill_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<distill_announcements::RemoteAnnouncement>> {
    let current = distill_announcements::filter_expired_at(
...
struct ResidentResources {
    ...
    codebase_index: Option<std::sync::Arc<distill_codebase_graph::IndexManagerHandle>>,
    is_headless: bool,
...
pub struct MvpAgent {
    ...
    pub(crate) interactive_auth: distill_login::single_flight::AuthSingleFlight,
    ...
    subagent_event_tx: distill_tools::implementations::distill::task::backend::SubagentCoordinatorSender,
    ...
    subagent_event_rx: RefCell<
        Option<
            distill_tools::implementations::distill::task::coordinator::SubagentCoordinatorReceiver,
        >,
    >,
    ...
    monitor_event_buffer: distill_tools::implementations::distill::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: distill_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<distill_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<distill_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &distill_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<distill_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !distill_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = distill_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// So switching e.g. `Distill` to `Distill-plan` doesn't require rebuilding the harness. A rebuild would destroy a client-supplied `_meta.agentProfile`.
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use distill_agent::config::is_strict_harness_agent_type;
    match (
...
use distill_chat_state::conversation_util::replace_or_insert_system_head;
...
impl AuthRequestMeta {
    ...
    fn login_override(&self) -> distill_login::LoginTransportOverride {
        if self.use_oauth {
            distill_login::LoginTransportOverride::ForceLoopback
        } else {
            distill_login::LoginTransportOverride::None
        }
    }
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<distill_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(distill_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<distill_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        session_id: &acp::SessionId,
        updates_file_path: &Option<PathBuf>,
    ) -> Vec<tokio::sync::oneshot::Receiver<distill_acp_lib::AcpResult<()>>> {
        ...
        for task in &orphaned {
            let snapshot = distill_tools::types::TaskSnapshot {
    ...
    /// Check whether the user has access via remote settings `allow_access`. Non-Grok auth (API keys, enterprise) always passes.
    /// For Grok OAuth2 users, reads `allow_access` from remote settings (explicit `false` blocks; absent field fails open). When settings have not arrived yet the gate is provisionally open and re-resolved on arrival.
    pub(super) async fn enforce_grok_code_access(
        ...
        *self.allow_access_resolved_for.borrow_mut() = Some(auth.user_id.clone());
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings distill_access_gate)"
            );
            ...
            let refresh_ok = if claim_already_current {
                ...
                match self
                        ...
                        .refresh_chain_bounded(
                            distill_login::token_type::TokenType::OidcSession,
                            distill_login::manager::RefreshReason::ServerRejected,
                            distill_login::manager::BEST_EFFORT_REFRESH_TIMEOUT,
                        )
                        .await
                    {
                        Ok(_) => {
    ...
    pub(crate) fn auth_response_with_meta(&self) -> AuthenticateResponse {
        let (show_resolved_model, gate, subscription_tier) = {
            ...
            let gate = rs
                ...
                .filter(|m| !m.is_empty())
                .map(|message| distill_login::GateInfo {
        ...
        let meta = self
            ...
            .map(|auth| {
                ...
                let auth_meta = distill_login::AuthMeta {
    ...
    pub(super) fn start_search_index_once(&self) {
        ...
        if let crate::session::storage::search::IndexDecision::On(index) = self
            .search_index()
        {
            index.bootstrap_once(crate::util::distill_home::distill_home());
        }
    }
    ...
    pub(super) fn spawn_auto_worktree_gc(&self) {
        ...
        let distill_home = distill_fast_worktree::resolve_distill_home();
    ...
    pub(super) fn reclaim_worktrees(
        distill_home: anyhow::Result<std::path::PathBuf>,
        policy: distill_fast_worktree::ResolvedWorktreeAutoGc,
    ) {
        if let Err(e) = distill_home
            .and_then(|home| distill_fast_worktree::WorktreeDb::open(&home))
            .and_then(|db| distill_fast_worktree::maybe_auto_gc(&db, &policy))
        {
...
async fn handle_synthetic_turn_trace(
    ...
    let turn_messages: Option<distill_chat_state::TurnCapture> = {
