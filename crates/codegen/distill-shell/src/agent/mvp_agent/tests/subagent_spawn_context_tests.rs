...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use distill_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = distill_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) = distill_workspace::permission::spawn_permission_manager(
                sid.clone(),
                gateway,
                cwd,
                distill_workspace::permission::types::ClientType::Generic,
                Some(
            ...
            for access in [
                distill_workspace::permission::AccessKind::Read(Some(".env".into())),
                distill_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                let decision = inherited
                    .request(distill_workspace::permission::PermissionRequest {
                        ...
                        ..distill_workspace::permission::PermissionRequest::new(
                            access.clone(),
                ...
                assert!(
                    matches!(
                        decision,
                        distill_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = distill_sampling_types::ToolOverrides {
        x_search: Some(distill_sampling_types::XSearchOptions {
            date_bound: Some(
                distill_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
...
async fn subagent_spawn_context_inherits_parent_process_scope() {
    ...
    let mut handle = make_test_handle("test-model", false, None);
    let parent_scope = distill_tty_utils::ProcessScope::new();
    handle.tool_context.process_scope = Some(parent_scope.clone());
    agent.insert_resident(&sid, handle);
    let owner = std::sync::Arc::new(distill_tty_utils::ProcessGroup::new().expect("process group"));
...
fn subagent_spawn_context_resolves_compaction_mode_like_parent() {
    ...
    use distill_chat_state::{CompactionDetail, CompactionMode};
