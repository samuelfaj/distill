...
fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
    use distill_sampling_types::ReasoningEffortOption;
    use indexmap::IndexMap;
...
fn reload_from_disk_cache_ignores_stale_cache() {
    ...
    let stale = ModelsCache {
        ...
        renewed_at: None,
        grok_version: Some(distill_version::VERSION.to_string()),
...
fn build_prefetched_map_distinct_ids_same_slug() {
    let entries = vec![
        make_entry_config_with_id(Some("auto"), "Distill", Some("Auto")),
        make_entry_config_with_id(Some("Distill"), "Distill", Some("Distill")),
    ...
    assert!(map.contains_key("Distill"));
    assert!(map.contains_key("experimental-fast"));
    let Some(auto) = map.get("auto") else {
        panic!("expected auto: {map:?}");
    };
    assert_eq!(
        auto.info.model, "Distill",
        "auto entry should still route to Distill"
    );
    let Some(build) = map.get("Distill") else {
        panic!("expected Distill: {map:?}");
    };
    assert_eq!(build.info.model, "Distill");
}
...
fn build_prefetched_map_duplicate_id_overwrites() {
    let entries = vec![
        make_entry_config_with_id(Some("Distill"), "Distill", Some("First")),
        make_entry_config_with_id(Some("Distill"), "Distill", Some("Second")),
    ];
    let map = build_prefetched_map(entries, None);
...
fn resolve_default_model_prefers_id_over_model_slug() {
    ...
    catalog.insert("auto-Distill".to_string(), make_model_entry("Distill"));
    catalog.insert("Distill".to_string(), make_model_entry("Distill"));

    let mut cfg = config::Config::default();
    cfg.models.default = Some("Distill".to_string());

    let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
    assert_eq!(key, "Distill", "must match id, not first slug hit");
}
...
fn resolve_catalog_key_maps_routing_slug_to_config_key() {
    ...
    assert_eq!(key.0.as_ref(), "enterprise-Distill");
}
...
fn resolve_catalog_key_last_slug_match_wins() {
    let mut models = IndexMap::new();
    models.insert("default-Distill".to_string(), make_model_entry("grok-4.5"));
    models.insert("user-Distill".to_string(), make_model_entry("grok-4.5"));

    let persisted = acp::ModelId::new("grok-4.5");
    let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
    assert_eq!(key.0.as_ref(), "user-Distill");
}
...
fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
    let mut models = IndexMap::new();
    models.insert("Distill".to_string(), make_model_entry("Distill"));
    models.insert(
        "enterprise-Distill".to_string(),
        make_model_entry("Distill"),
    );
    models.insert("grok-4.3".to_string(), make_model_entry("grok-4.3"));

    let available = test_available_keys(&["enterprise-Distill", "grok-4.3"]);

    let persisted = acp::ModelId::new("Distill");
    assert_eq!(
...
fn selectable_prefers_exact_key_over_later_slug_match() {
    let mut models = IndexMap::new();
    models.insert("Distill".to_string(), make_model_entry("grok-4.5"));
    models.insert("other".to_string(), make_model_entry("Distill"));

    let available = test_available_keys(&["Distill", "other"]);

    let persisted = acp::ModelId::new("Distill");
    let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
        .expect("exact selectable key must win");
    assert_eq!(key.0.as_ref(), "Distill");
}
...
fn personal_offline_boot_does_not_emit_a_managed_degraded_warn() {
    ...
    use distill_telemetry::unified_log::LogLevel;
