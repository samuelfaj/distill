...
//! Codex login, refresh, logout, and quota failures must never mutate Grok's
...
pub const CODEX_AUTH_REQUIRED_ERROR_KIND: &str = "distill_codex_auth_required";
pub const CODEX_AUTH_REQUIRED_MESSAGE: &str = "ChatGPT authentication required; run `distill login --chatgpt` or configure an API key for this model";
...
const CODEX_AUTH_ANCHOR_HEADER: &str = "x-distill-codex-auth-anchor";
const CODEX_ACCOUNT_ANCHOR_HEADER: &str = "x-distill-codex-account-anchor";
const CODEX_USER_ANCHOR_HEADER: &str = "x-distill-codex-user-anchor";
const CODEX_WORKSPACE_ANCHOR_HEADER: &str = "x-distill-codex-workspace-anchor";
...
impl CodexCredentials {
    pub(crate) fn identity(&self) -> CodexAuthIdentity {
        CodexAuthIdentity {
...
pub fn auth_file_path() -> PathBuf {
    crate::util::distill_home::distill_home().join(CODEX_AUTH_FILE_NAME)
}
...
/// resolver or by a provider-local OpenAI API key. Grok's auth cell never
...
async fn callback_handler(
    ...
    if query.state.as_deref() != Some(state.expected_state.as_str()) {
        return Html(
            "<!doctype html><title>Distill login failed</title><h1>ChatGPT login failed</h1><p>Return to Distill for details.</p>",
        );
    }
    ...
    if success {
        Html(
            "<!doctype html><title>Distill connected</title><h1>ChatGPT connected</h1><p>You can close this window and return to Distill.</p>",
        )
    } else {
        Html(
            "<!doctype html><title>Distill login failed</title><h1>ChatGPT login failed</h1><p>Return to Distill for details.</p>",
...
async fn run_device_login_at(path: &Path, endpoints: &CodexEndpoints) -> Result<CodexCredentials> {
    ...
    eprintln!("Continue only if you started this login in Distill.");
...
async fn refresh_at(
    ...
    if tokens.refresh_token.trim().is_empty() {
        bail!("ChatGPT OAuth refresh token is missing; run `distill login --chatgpt`");
    }
    ...
    if !status.is_success() {
        ...
        let action = if permanent {
            " Run `distill login --chatgpt` to reconnect."
        } else {
            ""
        };
        let message = format!(
...
pub async fn fetch_usage() -> Result<CodexUsageSnapshot> {
    ...
    let Some(mut credentials) = fresh_credentials().await? else {
        bail!("Not connected; run `distill login --chatgpt`");
    };
    match fetch_usage_with_credentials(&endpoints, &credentials).await {
        ...
        Err(UsageRequestError::Unauthorized) => {
            credentials = force_refresh()
                .await?
                .ok_or_else(|| anyhow!("Not connected; run `distill login --chatgpt`"))?;
...
/// by this or another Distill process.
...
impl CodexBearerResolver {
    ...
    fn resolve_credentials(
        ...
        Some(distill_sampler::config::ResolvedBearerAuth {
...
impl distill_sampler::BearerResolver for CodexBearerResolver {
...
impl distill_tools::types::ApiKeyProvider for CodexImageApiKeyProvider {
    fn current_api_key(&self) -> Option<String> {
        distill_sampler::BearerResolver::current_bearer(&self.resolver)
    }
}

pub(crate) fn image_api_key_provider(
    credentials: &CodexCredentials,
) -> distill_tools::types::SharedApiKeyProvider {
...
/// The resolver for [`distill_sampler::SamplerConfig::bearer_resolver`] on the
/// Codex backend: the OAuth token, refreshed before it expires.
pub fn codex_bearer_resolver() -> distill_sampler::config::SharedBearerResolver {
...
pub fn apply_codex_backend(cfg: &mut distill_sampler::SamplerConfig) {
    if !is_codex_backend(&cfg.base_url) {
        return;
    }
    if cfg.api_backend == distill_sampler::ApiBackend::default() {
        cfg.api_backend = distill_sampler::ApiBackend::Responses;
    }
    let credentials = load_credentials().ok().flatten();
...
mod tests {
    ...
    async fn permanent_refresh_failure_prevents_repeat_oauth_calls_until_file_rotation() {
        ...
        assert!(first.to_string().contains("login --chatgpt"));
