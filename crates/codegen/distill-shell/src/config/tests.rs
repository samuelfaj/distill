...
fn subagent_limit_counts_resolve_env_over_toml_over_remote_over_default() {
    use distill_tools::implementations::distill::task::admission;
...
fn subagent_sampling_limit_applies_precedence_and_clamps() {
    ...
    use distill_tools::implementations::distill::task::admission::DEFAULT_MAX_CONCURRENT;
...
#[test]
fn model_overrides_default_image_description_is_distill() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_distill() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn media_gen_caps_resolve_env_over_toml_over_remote_over_default() {
    use distill_tools::media_gen_limits::{
...
fn project_config_never_sources_feedback_user() {
    ...
    std::fs::write(
        ...
        .unwrap();
    let cwd = repo.path();
    distill_workspace::folder_trust::grant_folder_trust(cwd);
    assert!(
...
#[test]
fn validate_hooks_path_rejects_outside_distill_home() {
...
fn validate_hooks_path_rejects_traversal_attack() {
    ...
    let traversal = format!("{}/../evil", distill_home.display());
...
fn validate_hooks_path_accepts_grok_hooks_subdir() {
    let distill_home = crate::util::distill_home::distill_home();
    let valid_path = distill_home.join("hooks").join("my-hooks");
...
fn managed_settings_disables_features_and_requirements_overrides() {
    use crate::agent::config::Feature;
    use distill_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> distill_test_support::EnvGuard {
    distill_test_support::EnvGuard::set(distill_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use distill_agent::plugins::{TrustStore, discover_plugins};
    use distill_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROK_HOME", home.path());
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
