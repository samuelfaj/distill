...
//!
//! Fetches the authenticated user's Distill billing configuration (credit limit, usage, on-demand cap, billing period, history) from the backend.
...
/// Current billing configuration for Distill coding credits. Carries the newer credits-config fields (`credit_usage_percent`, `current_period`).
/// It also carries the deprecated `DistillBillingConfig` fields (`monthly_limit`, `used`, `billing_period_*`). Consumers should prefer the new fields and fall back to the deprecated ones.
/// The same struct then works against both the new `GetGrokCreditsConfig` and the legacy `GetDistillBillingConfig` responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// `None` when absent (legacy `GetDistillBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-XAI-Token-Auth",
            distill_login::GrokComConfig::default().token_header,
        )
