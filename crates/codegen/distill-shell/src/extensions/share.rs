...
use distill_telemetry::id::agent_id;
...
async fn upload_share_data_to_gcs(
    ...
    if let Err(e) = distill_file_utils::gcs::upload_bytes_signed(
...
fn require_xai_auth_for_share(
    auth_manager: &distill_login::AuthManager,
) -> Result<distill_login::GrokAuth, acp::Error> {
...
mod tests {
    ...
    use distill_login::GrokComConfig;
    ...
    fn make_auth_manager_with_token_expiring_in(
        ttl: Duration,
    ) -> (Arc<distill_login::AuthManager>, tempfile::TempDir) {
        let dir = tempdir().expect("tempdir for share auth test");
        let mgr = Arc::new(distill_login::AuthManager::new(
            dir.path(),
        ...
        // We must explicitly set oidc_issuer to a first-party Grok issuer.
    ...
    fn share_rejects_non_xai_auth_with_actionable_grok_login_message() {
        ...
        // API key is the simplest non-Grok credential (External and enterprise OIDC are also rejected the same way)
        ...
        let err = require_xai_auth_for_share(&mgr).expect_err(
            "non-Distill accounts (API key, External, enterprise IdP) must be rejected",
        );

        // Test the *exact* actionable data string for the non-Grok path (distinct from the generic "Authentication required to share session" path)
        let serialized =
