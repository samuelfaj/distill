...
async fn hubless_workspace_exposure_arms_no_metric_pump_and_tears_down_cleanly() {
    let handle = distill_workspace::WorkspaceHandle::for_test();
...
fn event_seq_of_parses_acp_and_ext_and_handles_missing() {
    ...
    // ExtNotification (Grok): nested under params.params._meta.eventId.
    let ext = pv(
        r#"{"params":{"method":"x.ai/session/update","params":{"sessionId":"019e-aa","_meta":{"eventId":"019e-aa-7"}}}}"#,
    );
    assert_eq!(event_seq_of(&ext), Some(7));
    // No eventId (Grok one-shot / older shell) yields None, so dedup cannot drop the event
...
fn inject_capabilities_adds_status_line_when_it_is_the_only_capability() {
    ...
    assert_eq!(
        j(&json, "/params/_meta").get(distill_status_line::CLIENT_STATUS_LINE_META),
...
async fn leader_client_id_dropped_when_target_disconnected() {
    ...
    // The direct ACP shape carries `params._meta`; the nested ext/notification shape (the Distill envelope) carries `params.params._meta`
    response_tx
        .send(format!(
