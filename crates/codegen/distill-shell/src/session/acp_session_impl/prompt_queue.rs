...
use super::*;
use distill_agent_lifecycle::ShutdownPolicy;
...
impl SessionActor {
    ...
    /// Merge consecutive plain prompts into `pending[0]` via [`distill_prompt_queue::combine_prefix_len`].
    ...
    pub(super) fn combine_front_pending_inputs(
        ...
        use distill_prompt_queue::{CombineGate, combine_prefix_len};
    ...
    fn combine_gate(item: &InputItem) -> distill_prompt_queue::CombineGate<'_> {
        ...
        let text = item
            ...
            .map(|m| m.text.as_str())
            .unwrap_or("");
        distill_prompt_queue::CombineGate {
    ...
    fn joined_text_blocks(blocks: &[acp::ContentBlock]) -> String {
        use distill_prompt_queue::join_texts;
    ...
    fn append_text_to_prompt(item: &mut InputItem, extra: &str) {
        use distill_prompt_queue::TEXT_SEPARATOR;
    ...
    fn stamp_combined_display_texts_meta(item: &mut InputItem) {
        use distill_prompt_queue::stamp_combined_display_texts;
