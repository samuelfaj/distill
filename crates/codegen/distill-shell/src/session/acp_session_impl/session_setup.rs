...
impl SessionActor {
    ...
    ) -> Option<distill_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
    ...
    pub(crate) async fn slash_skills_for_resolve(
        &self,
    ) -> Vec<distill_tools::implementations::skills::types::SkillInfo> {
        #[cfg(test)]
    ...
    pub(super) async fn wrap_skill_reminder(
        &self,
        effects: &distill_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn distill_auth::AuthCredentialProvider> = Arc::new(
        ...
        let (response, stamp) =
            match distill_auth::execute_with_stamp(&middleware_client, built).await {
                Ok(r) => r,
                Err(e) => {
        ...
        if response.status() == reqwest::StatusCode::UNAUTHORIZED {
            distill_login::attribution::record_consumer_401(
                am,
                None,
                distill_login::attribution::ConsumerKind::IdleResumeModelRefresh,
                "",
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = distill_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(distill_tools::types::resources::DenyReadGlobs(
    ...
    pub(super) async fn build_session_info(&self) -> SessionInfoData {
        ...
        let system_prompt_tokens = system_message
            .as_ref()
            .map(distill_chat_state::estimate_system_message_tokens)
            .unwrap_or(0);
        ...
        let tool_definitions_tokens =
            distill_chat_state::estimate_tool_definitions_tokens(&tool_defs);
        ...
        let free_tokens = distill_token_estimation::free_tokens(context_window, total_tokens);
        let usage_pct = distill_token_estimation::usage_percentage_u8(total_tokens, context_window);
