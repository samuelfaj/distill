...
use distill_telemetry::region;
use distill_telemetry::region::Parent as SpanParent;
use distill_telemetry::subagent_spawn::phase_region_under;
use tracing::Instrument;
static SESSIONS_ACTIVE: distill_telemetry::activity::ActivityGauge =
    distill_telemetry::activity::ActivityGauge::residency(
        distill_telemetry::activity::SESSIONS_ACTIVE_KEY,
    );
...
fn drop_cli_catchall_allows(
    rules: Vec<distill_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<distill_workspace::permission::types::PermissionRule>,
    Vec<distill_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if distill_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
/// Only caps the blocking pool; pre-warming is reserved for process-lifetime runtimes (`distill_tty_utils::runtime`).
pub(crate) fn build_session_runtime() -> std::io::Result<tokio::runtime::Runtime> {
    let mut builder = tokio::runtime::Builder::new_current_thread();
    distill_tty_utils::runtime::apply_blocking_pool(builder.enable_all()).build()
}
fn configured_memory_retrieval_mode(
    config: Option<&crate::config::MemoryConfig>,
) -> distill_telemetry::events::MemoryRetrievalMode {
    use distill_telemetry::events::MemoryRetrievalMode::*;
...
fn subagent_sampler_rate_limit_threshold(is_subagent: bool, pacer_max_attempts: u32) -> u32 {
    if is_subagent && pacer_max_attempts > 0 {
        distill_sampler::RATE_LIMIT_RETRY_DISABLED
    } else {
        distill_sampler::RATE_LIMIT_RETRY_THRESHOLD
    }
}
...
mod cli_catchall_drop_tests {
    ...
    use distill_workspace::permission::resolution::YoloPinReason;
    const PIN: &str = YoloPinReason::DisableBypassPermissionsMode.message();
    use distill_workspace::permission::rules::parse_permission_rule;
    use distill_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
    ...
    fn disabled_memory_config_has_disabled_retrieval_mode() {
        assert_eq!(
            ...
            distill_telemetry::events::MemoryRetrievalMode::Disabled
        );
    }
    #[test]
...
mod session_retry_policy_tests {
    ...
    use distill_sampler::{RATE_LIMIT_RETRY_DISABLED, RATE_LIMIT_RETRY_THRESHOLD};
    #[test]
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: distill_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<distill_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: distill_chat_state::CompactionMode,
    ...
    compat: CompatConfig,
    paths_config: distill_agent::prompt::paths::PathsConfig,
    ...
    web_search_sampling_config: Option<distill_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: distill_tools::implementations::distill::app_builder::AppBuilderDeployerConfig,
    write_file_enabled: bool,
    active_agent_messages_enabled: bool,
    agent_message_sender: Option<
        distill_tools::implementations::distill::task::types::AgentMessageSender,
    >,
    goal_enabled: bool,
    ...
    media_gen_batch_limits: distill_tools::media_gen_limits::MediaGenBatchLimits,
    ...
    plugin_registry_handle: Option<distill_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::remote_config::ModelsManager,
    inherited_permission_handle: Option<distill_workspace::permission::PermissionHandle>,
    api_key_provider: Option<distill_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<distill_hooks::discovery::HookRegistry>>,
    workspace_ops: distill_workspace::WorkspaceOps,
    cli_permission_rules: Vec<distill_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn distill_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        distill_tools::implementations::distill::scheduler::types::SchedulerHandle,
    >,
    max_turns: Option<usize>,
    forked_tool_override: Option<Vec<ToolSpec>>,
    is_chat_kind: bool,
    spawn_ctx: Option<distill_telemetry::subagent_spawn::SpawnPhaseContext>,
    sampling_gate: Option<Arc<tokio::sync::Semaphore>>,
) -> Result<(SessionInitResult, tokio::sync::oneshot::Receiver<()>), distill_agent::AgentBuildError>
{
    if max_turns == Some(0) {
        return Err(distill_agent::AgentBuildError::InvalidConfig(
    ...
    let (permissions, permission_events_rx, deny_read_globs) = async {
        use distill_workspace::permission::resolution as permission_resolution;
        if let Some(handle) = inherited_permission_handle {
                ...
                match &mut permission_config {
                    ...
                    None => {
                        permission_config = Some(
                            distill_workspace::permission::types::PermissionConfig::new(
                                cli_permission_rules,
                            ),
                        );
                    }
            ...
            let deny_read_globs = permission_config
                ...
                .unwrap_or_default();
            let hub_permission = if distill_workspace::permission::hitl_permission_live_enabled() {
                let server = match workspace_ops.workspace_handle() {
                    Some(handle) => handle.hub_server_blocking().await,
                    None => None,
                };
                let transport = server
                    .and_then(|server| distill_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                    ...
                    .map(|t| {
                        std::sync::Arc::new(t)
                            as std::sync::Arc<
                                dyn distill_workspace::permission::PermissionHookTransport,
                            >
                    });
            ...
            let (permissions, permission_events_rx) = distill_workspace::permission::spawn_permission_manager_with_pin(
    ...
    let web_search_config = if disable_web_search {
        distill_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            distill_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let resolved_max_retries = distill_sampler::resolve_max_retries(session_max_retries_source(
    ...
    let chat_state_sampling_config = distill_sampling_types::SamplingConfig {
    ...
    let actor_pruning_config = distill_chat_state::PruningConfig {
    ...
    let task_completion_reservations =
        distill_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        distill_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        distill_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        distill_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn distill_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn distill_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn distill_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(distill_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(distill_tools::computer::local::LocalFs)
        };
    ...
    let compaction_policy = distill_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        distill_tools::implementations::distill::ask_user_question::types::UserQuestionRequest,
    >();
    let attribution_callback_for_spec = auth_manager.as_ref().map(|am| {
        distill_login::attribution::ShellAttribution::new_tool_callback(
            am.clone(),
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn distill_tools::types::memory_backend::MemoryBackend>,
        ...
        .filter(|storage| storage.mode().is_legacy())
    {
        ...
        let embed_credentials = distill_login::credential_provider::embedding_session_credentials(
            &embed_base_url,
            auth_manager.as_ref(),
        ...
        let backend: std::sync::Arc<dyn distill_tools::types::memory_backend::MemoryBackend> =
        ...
        distill_telemetry::session_ctx::log_event(
            distill_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                memory_mode: distill_telemetry::memory_telemetry::MemoryMode::Legacy,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
        ...
        if let Some(storage) = memory_storage_for_session.as_ref() {
            ...
            distill_telemetry::session_ctx::log_event(
                distill_telemetry::memory_telemetry::MemorySessionInit {
                    session_id: session_info.id.to_string(),
                    memory_enabled: true,
                    memory_mode: distill_telemetry::memory_telemetry::MemoryMode::V2,
            ...
            let rollout = match controls.rollout {
                crate::config::MemoryV2Rollout::Off => {
                    distill_telemetry::memory_telemetry::MemoryV2Rollout::Off
                }
                crate::config::MemoryV2Rollout::RecordOnly => {
                    distill_telemetry::memory_telemetry::MemoryV2Rollout::RecordOnly
                }
                crate::config::MemoryV2Rollout::Shadow => {
                    distill_telemetry::memory_telemetry::MemoryV2Rollout::Shadow
                }
                crate::config::MemoryV2Rollout::Active => {
                    distill_telemetry::memory_telemetry::MemoryV2Rollout::Active
                }
            };
            distill_telemetry::session_ctx::log_event(
                distill_telemetry::memory_telemetry::MemoryV2ControlsPinned {
                    rollout,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        distill_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let (harness_metrics, scheduler_handle_for_handle, toolset) = async {
        ...
        let harness_metrics = if !startup_hints.is_subagent
            && (telemetry_enabled || distill_telemetry::external::is_active())
        {
            ...
            Some(super::telemetry::SessionHarnessMetrics {
                ...
                permission_mode: if session_yolo_mode {
                    distill_telemetry::enums::PermissionMode::AlwaysApprove
                } else if session_auto_mode
                    && crate::util::config::auto_permission_mode_enabled_from_disk()
                {
                    distill_telemetry::enums::PermissionMode::Auto
                } else {
                    distill_telemetry::enums::PermissionMode::Ask
                },
        ...
        let resolved_task_output =
            distill_tools::reminders::task_completion::resolve_task_output_tool_name(
                agent.tool_bridge(),
            )
            .await;
        let resolved_read =
            distill_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
                .await;
        let resolved_scheduler_delete =
            distill_tools::reminders::task_completion::resolve_scheduler_delete_tool_name(
                agent.tool_bridge(),
            )
            .await;
        let resolved_scheduler_create =
            distill_tools::reminders::task_completion::resolve_scheduler_create_tool_name(
                agent.tool_bridge(),
            )
            .await;
        ...
        tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
            distill_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
        });
        ...
        let scheduler_handle_for_handle = {
            ...
            res.get::<distill_tools::implementations::distill::scheduler::types::SchedulerHandle>()
                .cloned()
        };
        let toolset = agent.tool_bridge().toolset();
    ...
    let resolved_workspace_root = distill_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = distill_sampler::RetryPolicy {
    ...
    let sampler_handle = distill_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<distill_hooks::error::HookError> = Vec::new();
    let hooks_discovery_span = tracing::info_span!("spawn.hooks_discovery").entered();
    let built_hook_registry: Option<Arc<distill_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = distill_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        distill_tools::implementations::distill::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        distill_tools::implementations::distill::workflow::WorkflowLaunchEnvelope,
    >();
    ...
    let obs_bridge = {
        let sid = distill_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| distill_tool_protocol::SessionId::new("unknown").expect("valid"));
        distill_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<distill_sampling_types::ToolOverrides>,
    ...
    let vcs_kind = {
        ...
        match distill_workspace::session::git::discover_git_root(root) {
            distill_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                distill_workspace::session::git::detect_vcs_kind(&git_root)
            }
            _ => distill_workspace::session::git::VcsKind::None,
        }
    };
    ...
    let repo_status_plan = if matches!(vcs_kind, distill_workspace::session::git::VcsKind::None) {
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        hook_disabled: std::cell::RefCell::new(Arc::new(
            distill_hooks::trust::DisabledHooks::load(),
        )),
    ...
    async {
        ...
        }
        {
            ...
            session
                ...
                .borrow()
                .tool_bridge()
                .update_resource(distill_tools::types::resources::PlanFilePath(plan_path))
                .await;
        }
        ...
        session
            ...
            .tool_bridge()
            .update_resource(
                distill_tools::implementations::distill::workflow::WorkflowLaunchHandle(
        ...
        if !background_workflows_enabled {
            session
                ...
                .tool_bridge()
                .update_resource(
                    distill_tools::implementations::distill::update_goal::GoalUpdateHandle(
        ...
        .filter(|storage| storage.mode().is_legacy())
    {
        ...
        let reindex_handle = tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                distill_telemetry::session_ctx::log_event(
                    distill_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use distill_tools::implementations::distill::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = distill_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            distill_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    let hosting = SESSIONS_ACTIVE.enter();
    tokio::task::spawn_local(async move {
        let _hosting = hosting;
        distill_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
pub(crate) struct SessionInitResult {
    ...
    pub(crate) toolset: Arc<distill_tools::registry::types::FinalizedToolset>,
}
...
pub(crate) fn bind_installed_toolset(
    ...
    cwd: &std::path::Path,
    hunk_tracker: &distill_hunk_tracker::HunkTrackerHandle,
    toolset: &Arc<distill_tools::registry::types::FinalizedToolset>,
) -> Option<Arc<distill_tools::registry::types::FinalizedToolset>> {
...
pub(crate) async fn spawn_session_on_thread(
    ...
    spawn_ctx: Option<distill_telemetry::subagent_spawn::SpawnPhaseContext>,
    sampling_gate: Option<Arc<tokio::sync::Semaphore>>,
    spawn_trace: Option<distill_telemetry::startup::SpawnTraceContext>,
) -> Result<(SessionInitResult, SessionThread), acp::Error> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, distill_agent::AgentBuildError>,
    >();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx
                        .send(Err(distill_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
                ...
                let _trace_span = parent_traceparent
                    ...
                    .map(|tp| {
                        let meta = serde_json::json!({ "traceparent": tp })
                            ...
                            .cloned()
                            .unwrap_or_default();
                        let span = distill_otel::span_from_meta_traceparent(&meta);
                        span.entered()
                    });
            ...
            rt.block_on(distill_telemetry::session_ctx::drain_at_session_exit());
        });
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    distill_sampling_types::conversation::ContentPart::Text { text }
