...
use distill_hooks::{dispatcher, result};
...
fn stop_entry_from_task(task: &distill_tools::types::TaskSnapshot) -> StopBackgroundTask {
    let command_text =
...
fn stop_entry_from_subagent(
    summary: &distill_tools::implementations::distill::task::types::ActiveSubagentSummary,
) -> StopBackgroundTask {
    StopBackgroundTask {
...
fn stop_cron_from_scheduled(
    task: &distill_tools::implementations::distill::scheduler::types::ScheduledTask,
) -> StopSessionCron {
    StopSessionCron {
        id: task.id.clone(),
        schedule: distill_tools::implementations::distill::scheduler::interval::interval_to_human(
            task.interval_secs,
        ),
...
impl SessionActor {
    ...
    pub(crate) async fn list_active_subagents(
        &self,
    ) -> Vec<distill_tools::implementations::distill::task::types::ActiveSubagentSummary> {
    ...
    async fn announce_keep_working(
        ...
        for block in blocks {
            ...
            distill_telemetry::session_ctx::log_event(distill_telemetry::events::HookBlocked {
                ...
                cause: distill_telemetry::events::HookBlockCause::StopBlocked,
            });
        }
        if blocks.is_empty() {
            for context in additional_context {
...
mod stop_gate_snapshot_tests {
    ...
    fn task_snapshot(
        kind: distill_tools::computer::types::TaskKind,
    ...
    fn subagent_summary_maps_to_stop_entry() {
        let summary = distill_tools::implementations::distill::task::types::ActiveSubagentSummary {
    ...
    fn scheduled_task_maps_to_stop_cron() {
        let task = distill_tools::implementations::distill::scheduler::types::ScheduledTask::new(
            300,
