    ...
    local
        .run_until(async {
            ...
            agent
                .tool_bridge()
                .update_resource(
                    distill_tools::implementations::distill::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
    ...
    local
        .run_until(async {
            ...
            *actor
            ...
            let cancelled_strip = distill_sampler::RequestId::from("cancelled-strip");
            let timed_out_strip = distill_sampler::RequestId::from("older-timeout-strip");
    ...
    local
        .run_until(async {
            ...
            let sampler_handle = distill_sampler::SamplerActor::spawn(
                cfg,
                distill_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<distill_acp_lib::AcpClientMessage>();
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(distill_sampling_types::UserItem {
                    content: vec![distill_sampling_types::ContentPart::Text { text: "hi".into() }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use distill_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    distill_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
