...
fn x_search_cutoff_update() -> distill_sampling_types::ToolOverridesUpdate {
    distill_sampling_types::ToolOverridesUpdate {
        x_search: Some(Some(distill_sampling_types::XSearchOptions {
            date_bound: Some(
                distill_sampling_types::SearchDateBound::new(None, Some("2024-03-15".to_string()))
                    .unwrap(),
            ),
        })),
        web_search: None,
...
async fn tool_overrides_update_applies_at_promotion_never_at_enqueue() {
    ...
    local
        .run_until(async {
            let (actor, _rx) = build_actor().await;
            let options = distill_sampling_types::XSearchOptions {
                date_bound: Some(
                    distill_sampling_types::SearchDateBound::new(
                        None,
            ...
            let set_update = || distill_sampling_types::ToolOverridesUpdate {
                x_search: Some(Some(options.clone())),
                web_search: None,
            };
            let expected = distill_sampling_types::ToolOverrides {
            ...
            actor.apply_tool_overrides_update(Some(distill_sampling_types::ToolOverridesUpdate {
                x_search: Some(None),
                web_search: None,
            }));
            assert_eq!(
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_backend_search(vec![distill_sampling_types::HostedTool::XSearch {
                    options: None,
                }])
                .await;
            ...
            *actor.tool_overrides.borrow_mut() = Some(expected.clone());
            ...
            assert_eq!(
                ...
                vec![distill_sampling_types::HostedTool::XSearch {
...
async fn unsupported_backend_search_sends_no_hosted_tool_on_either_channel() {
    ...
    local
        .run_until(async {
            let (actor, _rx) = build_actor().await;
            let configured = distill_sampling_types::WebSearchOptions {
                allowed_domains: None,
                excluded_domains: Some(vec!["reddit.com".to_string()]),
            };
            *actor.agent.borrow_mut() =
                test_agent_backend_search(vec![distill_sampling_types::HostedTool::WebSearch {
            ...
            assert_eq!(
                distill_sampling_types::extra_tool_entries(&actor.hosted_tools_for_turn()).len(),
                1
            );
            ...
            assert!(
                distill_sampling_types::extra_tool_entries(&actor.hosted_tools_for_turn())
                    .is_empty(),
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
            ...
            assert_eq!(
                actor.effective_hosted_tools(),
                vec![distill_sampling_types::HostedTool::WebSearch {
                    options: Some(configured.clone()),
                }],
            );

            let per_turn = distill_sampling_types::WebSearchOptions {
...
async fn agent_rebuild_republishes_the_configured_cutoff() {
    ...
    local
        .run_until(async {
            ...
            let seed = distill_sampling_types::ToolOverrides {
                x_search: Some(distill_sampling_types::XSearchOptions {
                    date_bound: Some(
                        distill_sampling_types::SearchDateBound::new(
                            None,
            ...
            let mut seeded = distill_agent::AgentDefinition::default_distill();
            seeded.tool_overrides = Some(seed.clone());
            actor
                .handle_rebuild_agent_for_definition(
                    seeded,
                    distill_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_owned(),
                )
                .await
...
async fn set_tool_overrides_publishes_the_inheritance_cell_before_any_turn() {
    ...
    local
        .run_until(async {
            ...
            let cutoff = distill_sampling_types::ToolOverrides {
                x_search: Some(distill_sampling_types::XSearchOptions {
                    date_bound: Some(
                        distill_sampling_types::SearchDateBound::new(
                            None,
        ...
        .run_until(async {
            ...
            *actor
            ...
            while let Ok(message) = gateway_rx.try_recv() {
                if let distill_acp_lib::AcpClientMessage::ExtNotification(args) = message
                    && args.request.method.as_ref()
                        == crate::session::prompt_queue::QUEUE_CHANGED_METHOD
                {
