...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        distill_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(distill_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
async fn apply_turn_image_budget_and_prune(
    chat_state: &distill_chat_state::ChatStateHandle,
    items: Vec<ConversationItem>,
) -> Vec<ConversationItem> {
    let items = distill_chat_state::image_budget::apply_image_budget(items).items;
...
impl SessionActor {
    ...
    async fn maybe_pre_compaction_flush(
        ...
        if distill_memory::flush::should_flush(
            total_tokens,
            context_window,
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            distill_telemetry::session_ctx::log_event(
                distill_telemetry::events::AutoCompactSuppressed {
                    reason: reason.into(),
                    estimated_tokens,
                    context_window,
                },
    ...
    fn user_facing_compact_error(raw: &str) -> String {
        ...
        const INTERNAL_PREFIXES: &[&str] = &[
            ...
            distill_compaction::sampler::SAMPLER_BUILD_FAILED_PREFIX,
            distill_compaction::sampler::SAMPLER_START_FAILED_PREFIX,
        ];
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    distill_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    distill_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if distill_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        let trigger_str = match trigger {
            distill_telemetry::events::CompactionTrigger::Manual => "manual",
        ...
        let compaction = distill_telemetry::events::CompactionScope::begin(
            distill_telemetry::events::CompactionBeginParams {
                ...
                compaction_mode: match self.compaction.compaction_mode {
                    distill_chat_state::CompactionMode::Summary => {
                        distill_telemetry::events::CompactionModeLabel::Summary
                    }
                    distill_chat_state::CompactionMode::Transcript => {
                        distill_telemetry::events::CompactionModeLabel::Transcript
                    }
                    distill_chat_state::CompactionMode::Segments(_) => {
                        distill_telemetry::events::CompactionModeLabel::Segments
                    }
                },
        ...
        self.dispatch_hook(
            distill_hooks::event::HookEventName::PreCompact,
            distill_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            distill_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let compaction_tool_tokens =
            distill_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<distill_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(distill_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<distill_sampling_types::HostedTool> =
            self.hosted_tools_for_turn();
        if lossy_input {
            simplified_messages = distill_chat_state::compaction_utils::fit_conversation_to_budget(
                simplified_messages,
        ...
        let fr_config = distill_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            ...
            .await
            {
                ...
                Err(distill_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                ...
                Err(distill_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(distill_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            distill_telemetry::session_ctx::log_event(
                                distill_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                CompactInputStage::VerbatimFitted => {
                                    let verbatim = distill_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                distill_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                distill_tools::computer::types::TaskKind::Bash => {
                    ...
                    let scheduled_loops = {
                        ...
                        .map(|t| {
                            ...
                            ScheduledLoopSummary {
                                task_id: t.id,
                                interval: distill_tools::implementations::distill::scheduler::interval::interval_to_human(
                                    t.interval_secs,
                                ),
                                next_fire_at,
                                prompt: t.prompt,
        ...
        let memory_ref: Option<&dyn distill_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn distill_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        distill_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: distill_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            distill_hooks::event::HookEventName::PostCompact,
            distill_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
        )
        ...
        compaction.complete(
            distill_telemetry::events::CompactionCompleteStats {
                tokens_after,
                two_pass_used,
                segments_queued,
            ...
            distill_telemetry::events::CompactionTiming {
    ...
    pub(crate) async fn run_compact_only(
        ...
        distill_telemetry::session_ctx::log_event(distill_telemetry::events::AutoCompactFired {
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<distill_sampling_types::ToolSpec>,
        user_context: Option<&str>,
