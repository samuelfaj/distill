...
pub(crate) struct RoleToolNames {
    ...
    /// `ToolKind::Edit` is the default Distill host's `search_replace` mutator.
    pub write: String,
...
impl RoleToolNames {
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default Distill host's pre-spawn describe probe exposes only `Edit` (`search_replace`) as the file mutator
...
pub(crate) mod tests {
    ...
    #[test]
    fn from_summary_uses_distill_names() {
        // A Distill toolset: client names match the literal defaults.
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // So the planner prompt still names a real tool on the default Distill host (the stock `web_search`/`web_fetch`)
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_distill_host() {
        // Default Distill host: the pre-spawn describe probe exposes only `Edit` (`search_replace`); `Write` is injection-only and absent
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default Distill parent bridge: no `Write`, only `Edit` (`search_replace`)
        let tn = RoleToolNames::from_parent(
            Some("read_file".into()),
            Some("list_dir".into()),
            Some("grep".into()),
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_distill_mutator() {
        // The explicit-pair `primary` render comes from `from_summary`; the inherit/fail-open `fallback` render comes from `from_parent`
        // Both must name the SAME mutator on the default Distill host (Edit-only toolset)
