...
//! Distill's L5 wiring onto the shared full-replace engine (`distill_compaction::code_compaction`).
//!
//! The shared engine drives the loop: sample, retry, then classify the result as degenerate or failed.
//! The loop lives in [`sample_full_replace_summary`](distill_compaction::sample_full_replace_summary).
//! This module adapts Distill's transport and telemetry to the engine's two traits: [`ShellCompactionSampler`] and [`ShellFullReplaceObserver`].
//!
//! The **input ladder** (verbatim, then fitted, then lossy) and auto-compaction suppression stay in L5 (`compaction.rs`).
//! Both are driven by the `context_overflow` / `deterministic` flags on [`FullReplaceError`](distill_compaction::FullReplaceError).
...
/// Wraps `generate_session_compact` as the shared engine's [`CompactionSampler`] for Distill's full-replace pass.
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // Append the harness-selected summarization prompt as the final user message (compat short vs structured Distill)
...
/// Map Distill's [`CompactFailure`] onto the shared engine's [`CompactionSampleError`].
...
/// [`FullReplaceObserver`] that reproduces Distill's per-attempt telemetry without the shared engine depending on a telemetry backend.
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    distill_telemetry::session_ctx::log_event(CompactionRetryDegraded {
