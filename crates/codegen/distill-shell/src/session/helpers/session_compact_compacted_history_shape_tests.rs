...
/// Thin wrapper around the shared `build_compacted_history` from `distill-chat-state`.
...
async fn test_compacted_history_raw_strings() {
    ...
    let formatted_summary =
        distill_chat_state::compaction_utils::format_compact_summary_content(compaction_summary);
    assert_eq!(
...
/// Regression guard: Distill must DROP the working tail post-compaction.
/// A prior change routed Distill to keep `recent_messages`, which survive only as `Tool call omitted...` stubs (dead tokens).
/// Mirrors `summary_before_recent_compaction_with_no_user_query_yields_three_messages` for Distill (`summary_before_recent = false`).
#[tokio::test]
async fn distill_compaction_drops_working_tail_regression_206460() {
    let conversation = vec![
        ConversationItem::system("You are a helpful assistant."),
        ConversationItem::user(
            "<user_info>\nOS: macos\n</user_info>\n\n<user_query>\nread auth.rs\n</user_query>",
    ...
    assert!(
        ...
        "Distill must drop recent_messages post-compaction",
    );
    assert!(
    ...
    assert!(
        !compacted
            ...
            .any(|i| matches!(i, ConversationItem::ToolResult(_))
                || i.text_content() == "Tool call omitted..."),
        "no tail (ToolResult or stub) may leak into the Distill compacted history",
    );
}
...
fn fallback_minimal_history_has_no_tool_results() {
    use distill_chat_state::compaction_utils::validate_compacted_history;
