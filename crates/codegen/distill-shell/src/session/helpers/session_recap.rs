...
use distill_chat_state::{compaction_utils, estimate_conversation_tokens, estimate_item_tokens};
...
pub(crate) fn build_instruction_items(
    ...
    let mut items = if strip_reasoning {
        distill_chat_state::compaction_utils::strip_reasoning_blocks(conversation)
    } else {
        conversation
    };
...
/// This is the verified `max_prompt_length` for current `Distill` / `grok-4.5` product backends (`500000`).
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use distill_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
        use std::sync::Arc;
    ...
    fn mk_tool_call(id: &str, args: &str) -> distill_sampling_types::ToolCall {
        use std::sync::Arc;
