...
use distill_workspace::session::file_state::RewindPoint;
...
pub enum PersistenceMsg {
    ...
    AppendCwdSwitchAndAck {
        ...
        respond_to: tokio::sync::oneshot::Sender<
            Result<distill_chat_state::StrictAppendAck, distill_chat_state::StrictAppendError>,
        >,
    },
    ...
    CurrentModel {
        ...
        /// The active agent definition name (e.g. `"Distill"`).
    ...
    UpgradeToWriteback {
        auth_manager: Arc<distill_login::AuthManager>,
    },
    Flush,
...
pub use distill_shared::session::session_dir;
...
pub fn session_exists_for_cwd(session_id: &str, cwd: &str) -> bool {
    let sessions_root = crate::util::distill_home::distill_home().join("sessions");
...
fn session_exists_for_cwd_in_root(session_id: &str, cwd: &str, sessions_root: &Path) -> bool {
    let encoded = crate::util::distill_home::encode_cwd_dirname(cwd);
...
pub fn resolve_local_session_ids_any_cwd<S: AsRef<str>>(
    ...
    resolve_local_session_ids_any_cwd_in_root(session_ids, &distill_home().join("sessions"))
...
pub(crate) fn resolve_local_session_any_cwd_result(session_id: &str) -> io::Result<Option<String>> {
    resolve_local_session_any_cwd_in_root(session_id, &distill_home().join("sessions"))
        .map_err(io::Error::other)
}
...
fn resolve_local_session_any_cwd_in_root(
    ...
    Ok(session_path
        .parent()
        .and_then(crate::util::distill_home::decode_cwd_from_dirname))
}
...
pub(crate) fn find_persisted_session_dir_by_id_result(
    ...
    find_persisted_session_dir_by_id_in_root_result(session_id, &distill_home().join("sessions"))
}
...
pub(crate) fn find_any_session_dir_by_id_result(session_id: &str) -> io::Result<Option<PathBuf>> {
    storage_view(&distill_home().join("sessions"))
...
pub(crate) fn find_summary_by_session_id(session_id: &str) -> Option<Summary> {
    ...
    find_summary_by_session_id_in_root(session_id, &distill_home().join("sessions"))
}
...
impl SessionKindIndex {
    pub(crate) fn load() -> io::Result<Self> {
        Self::load_in_root(&distill_home().join("sessions"))
    }
...
pub fn local_summaries_for_cwd_sync(
    ...
    local_summaries_for_cwd_sync_in_root(cwd, selection, &distill_home().join("sessions"))
}
...
pub fn resumed_session_sandbox_profile(
    ...
    resumed_session_sandbox_profile_in_root(session_id, cwd, &distill_home().join("sessions"))
}
...
pub fn resolve_recent_session_sandbox_profile(
    ...
    most_recent_local_summary_for_cwd_in_view(
        cwd?,
        &storage_view(&distill_home().join("sessions")).ok()?,
        read_summary_from_dir,
        selection,
    )
    .ok()
...
pub fn ensure_owner_only_session_dir(info: &Info) -> std::io::Result<PathBuf> {
    ensure_owner_only_session_dir_in(&distill_home(), info)
}

/// Inner implementation with an injectable grok home for tests.
fn ensure_owner_only_session_dir_in(distill_home: &Path, info: &Info) -> std::io::Result<PathBuf> {
...
fn ensure_owner_only_session_dir_in_with(
    ...
    let dir = session_dir_in(distill_home, info);
    crate::session::storage::create_dir_all_durable_with(
        ...
        |dir| {
            ...
            if let Ok(cwd_dir) =
                crate::util::distill_home::ensure_sessions_cwd_dir_in(distill_home, &info.cwd)
            {
                crate::session::storage::sync_cwd_marker_if_present_with(&cwd_dir, &sync_file)?;
            }
            crate::util::distill_home::create_dir_all_owner_only(dir)
        },
        sync_dir,
    )?;
    Ok(dir)
...
fn session_dir_in(distill_home: &Path, info: &Info) -> PathBuf {
    crate::util::distill_home::sessions_cwd_dir_in(distill_home, &info.cwd)
...
pub(crate) fn get_prompt_file_path(info: &Info, prompt_index: usize) -> PathBuf {
    get_prompt_file_path_in(&distill_home(), info, prompt_index)
}

/// Inner implementation with an injectable grok home for tests.
fn get_prompt_file_path_in(distill_home: &Path, info: &Info, prompt_index: usize) -> PathBuf {
    // Best-effort; failures surface on the prompt-file write itself.
    let _ = ensure_owner_only_session_dir_in(distill_home, info);
    let prompts_dir = session_dir_in(distill_home, info).join("prompts");
    let _ = crate::util::distill_home::create_dir_all_owner_only(&prompts_dir);
...
pub(crate) struct ExplicitSessionIdentity {
    pub(crate) agent_id: distill_message_delivery_core::AgentId,
    pub(crate) attempt_id: distill_message_delivery_core::AttemptId,
}
...
pub(crate) fn next_session_identity(
    ...
    let previous_agent_id =
        previous_agent_id.and_then(distill_message_delivery_core::AgentId::parse);
    let action = distill_message_delivery_core::next_identity(
    ...
    let (agent_id, attempt_id) = match action {
        distill_message_delivery_core::IdentityAction::Start { agent, attempt }
        | distill_message_delivery_core::IdentityAction::Resume { agent, attempt }
        | distill_message_delivery_core::IdentityAction::Fork { agent, attempt } => {
            (agent, attempt)
        }
    };
    SessionIdentity {
...
pub struct Summary {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub distill_home: Option<String>,
...
/// Current `distill_home` as a UTF-8 string, or `None` if the path isn't valid UTF-8.
pub(crate) fn distill_home_string() -> Option<String> {
...
impl Summary {
    pub(crate) fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            distill_workspace::session::git::resolve_persisted_session_git_metadata_sync(
        ...
        let mut summary = Self {
            ...
            request_id: None,
            distill_home: distill_home_string(),
            last_active_at: None,
...
impl SessionPersistence {
    ...
    async fn upgrade_to_writeback(&mut self, auth_manager: Arc<distill_login::AuthManager>) {
            ...
            match msg {
                ...
                PersistenceMsg::Update(update) => {
                    match update {
                        ...
                        SessionUpdate::Xai(_) => {
                            // Grok notifications are written directly without merging
                    ...
                    let result = self
                        ...
                        .map_err(|error| match error {
                            crate::session::storage::AppendCwdSwitchError::NotCommitted(error) => {
                                distill_chat_state::StrictAppendError::NotCommitted(error)
                            }
                            crate::session::storage::AppendCwdSwitchError::Committed {
                                acknowledgement,
                                source,
                            } => distill_chat_state::StrictAppendError::Committed {
                                acknowledgement,
                                source,
                            },
                        });
...
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = distill_config::distill_home().join("logs").join("mcp");
...
fn collect_session_files_recursive(base: &Path, dir: &Path, files: &mut Vec<CopiedSessionFile>) {
    let artifacts = distill_feedback::FeedbackDraftArtifactSet::for_session(base);
...
fn collect_session_files_recursive_with_artifacts(
    ...
    artifacts: &distill_feedback::FeedbackDraftArtifactSet,
) {
    ...
    for entry in entries.flatten() {
        if distill_feedback::is_feedback_draft_artifact_name(&entry.file_name()) {
            continue;
        }
        let path = entry.path();
...
/// Queue a fresh session's local-only ACP history to `remote_sync` (Grok updates are never synced), returning the count.
...
fn init_remote_sync(
    ...
    auth_manager: Option<Arc<distill_login::AuthManager>>,
...
pub async fn list_summaries(cwd: Option<&str>) -> io::Result<Vec<Summary>> {
    let root_dir = crate::util::distill_home::distill_home();
...
pub async fn delete_session_history(
    ...
    needs_remote: bool,
    auth_manager: Arc<distill_login::AuthManager>,
    search_index: Option<&distill_session_search::SearchIndexManager>,
...
pub(crate) fn cleanup_stale_sessions(live_session_dir: &Path) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        let sessions_root = distill_home().join("sessions");

        tracing::info!(
            target: "distill_shell::session::persistence",
