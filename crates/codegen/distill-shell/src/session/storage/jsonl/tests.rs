...
async fn write_compaction_segment_numbers_and_indexes_resume_safely() {
    ...
    let seg = |summary: &str| CompactionSegmentFile {
        ...
        detail: distill_chat_state::CompactionDetail::Verbose,
    ...
    let base = adapter
        .session_dir(&info)
        .join(distill_compaction_transcript::COMPACTION_DIR);
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use distill_paths::RelPathBuf;
    use distill_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use distill_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    distill_sandbox::set_configured_profile("workspace");
    let expected = distill_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn scan_session_dirs_finds_all_sessions() {
    ...
    let cwd = crate::util::distill_home::encode_cwd_dirname("/home/user/project");
...
fn scan_session_dirs_filters_by_cwd() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let cwd_a = crate::util::distill_home::encode_cwd_dirname("/home/user/project-a");
    let cwd_b = crate::util::distill_home::encode_cwd_dirname("/home/user/project-b");
...
fn scan_session_dirs_skips_non_directory_entries() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::distill_home::encode_cwd_dirname("/project");
...
fn scan_session_dirs_continues_when_a_cwd_bucket_is_gone() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let keep = crate::util::distill_home::encode_cwd_dirname("/keep");
...
async fn list_sessions_recent_returns_most_recent_by_mtime() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::distill_home::encode_cwd_dirname("/workspace");
...
async fn list_sessions_sorts_by_last_active_at_over_updated_at() {
    ...
    let cwd = crate::util::distill_home::encode_cwd_dirname(cwd_path);
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd,
...
async fn list_sessions_recent_sorts_by_updated_at() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::distill_home::encode_cwd_dirname("/ws");
    let now = chrono::Utc::now();
...
async fn list_sessions_recent_spans_multiple_workspaces() {
    let tmp = TempDir::new().unwrap();
    let cwd_a = crate::util::distill_home::encode_cwd_dirname("/project-a");
    let cwd_b = crate::util::distill_home::encode_cwd_dirname("/project-b");
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd_a,
...
/// Exercises the real on-read upgrade path end-to-end (loader, serde, and distill_sampling_types::upgrade_legacy_reasoning).
...
/// The assistant carries `reasoning: { text, encrypted, id }` inline, the legacy Distill / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"Distill"}"#,
        ],
    );
    assert_eq!(
            items.len(),
    ...
    match item {
        ConversationItem::Reasoning(r) => {
            ...
            let Some(distill_sampling_types::rs::SummaryPart::SummaryText(s)) = r
                .summary
                .first() else {
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"Distill"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"Distill"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    let Some(distill_sampling_types::rs::SummaryPart::SummaryText(s)) = reconstructed
        .summary
        .first() else {
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"Distill"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"Distill"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
...
async fn retry_after_lost_ack_converges_memory_and_disk_to_authoritative_item() {
    ...
    let chat = distill_chat_state::ChatStateActor::spawn(
        vec![],
    ...
    assert!(matches!(
            ...
            .await,
            Err(distill_chat_state::StrictAppendError::Indeterminate(_))
        ));
    ...
    assert!(matches!(
            ...
            .await
            .unwrap(),
            distill_chat_state::StrictAppendAck::AlreadyPresent(item)
...
async fn acknowledged_chat_append_preserves_existing_file_bytes_and_appends_once() {
    ...
    assert!(matches!(
            adapter
                ...
                .await
                .unwrap(),
            distill_chat_state::StrictAppendAck::Appended
        ));
...
async fn init_session_creates_owner_only_session_and_parent_dirs() {
    ...
    let session_dir = temp_dir
        .path()
        .join("sessions")
        .join(crate::util::distill_home::encode_cwd_dirname(&info.cwd))
...
async fn copy_session_data_creates_owner_only_target_dir() {
    ...
    let target_dir = temp_dir
        .path()
        .join("sessions")
        .join(crate::util::distill_home::encode_cwd_dirname(&target.cwd))
