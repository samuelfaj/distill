...
//! Everything downstream of the extracted string (hashing, dedup, the SQLite index itself) lives in `distill-session-search`.
...
pub(super) fn collect_all_indexable_content_single_pass(
    ...
    for line_result in reader.lines() {
        ...
        // Content events arrive on ACP "session/update"; control events (rewind markers) on the Distill "_x.ai/session/update" extension
        if !is_xai {
            match tag {
