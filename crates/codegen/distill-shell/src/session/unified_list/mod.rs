...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source_workspace_dir: Some("/Users/me/distill-src".into()),
    ...
    fn xai_auth_manager(dir: &std::path::Path) -> std::sync::Arc<distill_login::AuthManager> {
        let am = std::sync::Arc::new(distill_login::AuthManager::new(
            dir,
            distill_login::GrokComConfig::default(),
        ));
        am.hot_swap(distill_login::GrokAuth {
    ...
    async fn forced_kind_serves_conversations_only() {
        let addr = spawn_conversations_stub(
            ...
            .await;
        let _env = distill_test_support::EnvGuard::set(
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = distill_test_support::EnvGuard::unset("GROK_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = distill_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = distill_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        ...
        let _chat_off = distill_test_support::EnvGuard::unset(GROK_CHAT_MODE_ENV);
        let _desktop_off = distill_test_support::EnvGuard::unset("GROK_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                distill_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = distill_test_support::EnvGuard::set(GROK_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = distill_test_support::EnvGuard::unset(GROK_CHAT_MODE_ENV);
        ...
        {
            let _on = distill_test_support::EnvGuard::set(GROK_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = distill_workspace::session::git::normalize_repo_url(this_repo).unwrap();
        ...
        let rows = relax_rows(
            ...
            vec![
                ...
                summary("theirs", Some("git@github.com:example/other.git"), 9),
            ],
            30,
            facet_registry(),
            HeadlessPolicy::Exclude,
