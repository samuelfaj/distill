...
use distill_tools::implementations::distill::workflow::WorkflowControl;
use distill_workflow::{Journal, WorkflowOutcome, WorkflowRunParams};
...
pub(crate) const WORKFLOW_DEFAULT_AGENT_BUDGET: u64 = distill_workflow::DEFAULT_AGENT_BUDGET;

static WORKFLOW_RUNS_ACTIVE: distill_telemetry::activity::ActivityGauge =
    distill_telemetry::activity::ActivityGauge::work(
        distill_telemetry::activity::WORKFLOW_RUNS_ACTIVE_KEY,
    );
...
pub(crate) struct WorkflowManager {
    ...
    subagent_event_tx:
        mpsc::UnboundedSender<distill_tools::implementations::distill::task::types::SubagentEvent>,
...
impl WorkflowManager {
    ...
    pub(crate) fn launch(
        ...
        let (run_id, journal, state) = match &spec.resume_run_id {
            Some(run_id) => {
                ...
                if existing.status
                    ...
                    && existing.agents_used >= distill_workflow::MAX_AGENT_BUDGET
                {
                    return Err(LaunchError::NotResumable(
        ...
        let exec = tokio::task::spawn_blocking(move || {
            distill_workflow::run_workflow(WorkflowRunParams {
                script,
                args,
                journal,
                host_tx,
            ...
            if !epoch_matches {
                ...
                log_run_ended(
                    RunEndMetadata {
                        ...
                        status: distill_telemetry::events::WorkflowRunEndStatus::Superseded,
                        duration_ms: elapsed,
                        agents_used,
                        agent_budget,
                    },
            ...
            if state.is_none() {
                ...
                log_run_ended(
                    RunEndMetadata {
                        run_id: &watcher_run_id,
                        parent_session_id: &watcher_session_id,
                        status: distill_telemetry::events::WorkflowRunEndStatus::Interrupted,
                        duration_ms: 0,
                        agents_used: 0,
                        agent_budget: None,
                    },
    ...
    pub(crate) fn test_bundle_with_session_dir(
        ...
        let notify = super::notify::WorkflowNotifySender::new(
            ...
            distill_acp_lib::AcpAgentGatewaySender::new(gateway_tx),
            persist_tx,
            store.clone(),
        );
    ...
    fn cancel_children_for_run(&self, run_id: &str) -> bool {
        ...
        self.subagent_event_tx
            .send(
                distill_tools::implementations::distill::task::types::SubagentEvent::Cancel(
                    distill_tools::implementations::distill::task::types::SubagentCancelRequest {
                        parent_session_id: Some(self.session_id.clone()),
                        target: distill_tools::implementations::distill::task::types::SubagentCancelTarget::WorkflowRunId(
                            run_id.to_owned(),
                        ),
                        respond_to,
                    },
...
fn log_run_started(
    ...
    use distill_telemetry::events::{WorkflowRunStarted, WorkflowSourceKind};
    distill_telemetry::session_ctx::log_event(WorkflowRunStarted {
...
struct RunEndMetadata<'a> {
    ...
    status: distill_telemetry::events::WorkflowRunEndStatus,
    duration_ms: u64,
    agents_used: u64,
...
fn log_run_ended(episode: RunEndMetadata<'_>, stats: &super::host_service::WorkflowAgentStats) {
    distill_telemetry::session_ctx::log_event(distill_telemetry::events::WorkflowRunEnded {
...
) -> distill_telemetry::events::WorkflowRunEndStatus {
    use crate::session::workflow::tracker::WorkflowRunStatus as S;
    use distill_telemetry::events::WorkflowRunEndStatus as E;
    match status {
        S::Active => E::Active,
...
mod tests {
    ...
    type CancelLog = Arc<
        parking_lot::Mutex<
            Vec<distill_tools::implementations::distill::task::types::SubagentCancelTarget>,
        >,
    >;
    ...
    ) -> distill_tools::implementations::distill::task::types::SubagentSpawnRequest {
    ...
    fn complete_spawn(
        req: distill_tools::implementations::distill::task::types::SubagentSpawnRequest,
    ) {
        use distill_tools::implementations::distill::task::types::SubagentResult;
        let id = req.id.clone();
    ...
    async fn failed_run_resumes_and_reexecutes_failed_host_call_live() {
        ...
        assert!(
            std::fs::read_to_string(&journal_path)
                .unwrap()
                .contains("__distill_workflow_host_error"),
    ...
    async fn control_run_refuses_to_pause_an_engine_paused_run() {
        ...
        manager.tracker.lock().apply_outcome(
            &run_id,
            &WorkflowOutcome::Paused {
                kind: distill_workflow::PauseKind::BackOff,
