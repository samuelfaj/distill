...
use distill_workflow::{WorkflowMeta, extract_meta};
...
pub(crate) enum ResolveError {
    ...
    Meta(#[from] distill_workflow::MetaError),
}

pub(crate) fn project_root(session_cwd: &Path) -> PathBuf {
    distill_workspace::session::git::find_git_root_from_path(session_cwd)
        .unwrap_or_else(|_| session_cwd.to_path_buf())
}

pub(crate) fn user_workflow_dir() -> PathBuf {
    crate::util::distill_home::distill_home().join("workflows")
}
...
mod tests {
    ...
    fn inline_resolution_validates_name_and_size() {
        ...
        assert!(matches!(
            ...
            Err(ResolveError::Meta(distill_workflow::MetaError::InvalidName))
        ));
        assert!(matches!(
