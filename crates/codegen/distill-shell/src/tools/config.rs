...
impl BashToolConfig {
    ...
    pub(crate) fn to_bash_params_json(
        ...
        // Production Distill opts up to 10h by sending it explicitly (overridable via config.toml)
...
impl WebFetchToolConfig {
    ...
    pub(crate) fn resolve_params(
    ...
    ) -> distill_tools::implementations::distill::web_fetch::WebFetchParams {
        ...
        let allow_local = self
            .allow_local
            .or_else(|| distill_config::env_bool("GROK_WEB_FETCH_ALLOW_LOCAL"));
...
/// It is distinct from `distill_tools::registry::types::ToolsetConfig` which holds tool-implementation-level config (bash limits, web search mode).
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "DistillHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "DistillHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"Distill:read_file"));
        assert!(ids.contains(&"Distill:search_replace"));
        assert!(ids.contains(&"Distill:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"DistillHashline:hashline_read"));
        assert!(ids.contains(&"DistillHashline:hashline_edit"));
        assert!(ids.contains(&"DistillHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            distill_agent::config::AgentDefinition::plan(),
            distill_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "Distill:search_replace"),
            ...
            assert!(
                ids.contains(&"DistillHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"Distill:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"DistillHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production Distill must set the 10h foreground ceiling"
        );
    }
