...
use distill_tools::types::compat::{CompatConfig, CompatConfigToml};
...
pub use distill_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types moved to `distill-config-types`; the re-export keeps `crate::util::config::*` paths working
...
pub use distill_config_types::{McpConfig, RelaySyncConfig};
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    distill_config::distill_home().join("mcp_preferences.json")
}
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    distill_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
/// User `$GROK_HOME/config.toml`. [`config_path`] is live (`distill_home()` is OnceLock).
...
pub async fn delete_mcp_server_config_at(
    ...
    if wrote
        && let Ok(mut cred_store) = distill_mcp::credentials::McpCredentialStore::load_default()
    {
...
// `.mcp.json` discovery moved to `distill-workspace` (client-side, shared with the folder-trust gate)
// The re-export keeps `crate::util::config::*` paths working
pub use distill_workspace::project_config::{
...
pub fn load_cli_plugin_registry(cwd: &std::path::Path) -> distill_agent::plugins::PluginRegistry {
    ...
    let discovered = distill_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    distill_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
fn config_path() -> PathBuf {
    // Live `$GROK_HOME` first: `distill_home()` is OnceLock and misses EnvGuard/tests.
    distill_dirs::resolve_distill_home()
        .unwrap_or_else(crate::util::distill_home::distill_home)
        .join("config.toml")
}
...
pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    distill_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = distill_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = distill_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = distill_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
    ...
    fn load_cli_plugin_registry_includes_project_config_path_plugins() {
        ...
        let _env = distill_test_support::EnvGuard::set("GROK_HOME", home.path());
        ...
        let agents = distill_agent::discovery::plugin_agents(&registry);
        assert!(
            agents
                .iter()
...
"#;
    ...
    async fn restore_mcp_server_enabled_after_enable_scopes_tiers() {
        ...
        // Do not call save_mcp_server_enabled_in (that reads ambient config_path / distill_home)
