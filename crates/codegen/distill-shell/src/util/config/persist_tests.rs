...
fn merge_section_preserves_unmodeled_fields_inside_a_nested_table() {
    ...
    let cfg = crate::agent::config::UiConfig {
        status_line: distill_status_line::test_support::StatusLineConfigFixture::from_kind(
...
fn no_home_cwd_config_resolves_slot_not_follow() {
    ...
    distill_config::fs_atomic::require_same_follow_destination(&link, slot.as_path())
