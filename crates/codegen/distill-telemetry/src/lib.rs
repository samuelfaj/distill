...
//! Telemetry engine for Distill sessions.
//! Covers product events, Mixpanel emission, Sentry error reporting, OpenTelemetry tracing, and the structured unified log.
//!
//! Extracted from `distill-file-utils` so telemetry has its own ownership boundary (see CODEOWNERS).
...
pub(crate) use distill_otel::otlp;
// Shared redaction utils now live in the foundation crate; re-export keeps `crate::redact_common` paths working.
pub(crate) use distill_otel::redact_common;
pub use distill_otel::redact_common::redact_error_detail;
