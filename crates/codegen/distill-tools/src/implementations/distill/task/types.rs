...
use distill_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
impl SpawnRootSpan {
    pub fn new(span: tracing::Span) -> Self {
        let traceparent = distill_otel::span_traceparent(&span);
        Self {
            span: Some(span),
            traceparent,
        }
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `/goal`-only harness override: the `agent_type` (e.g. `"cursor"`, `"distill-plan"`) whose `AgentDefinition` decides the child's harness
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "Distill:run_terminal_cmd" | "DistillConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("distill", "SubagentEventSender", SubagentEventSender);
...
register_resource!("distill", "SubagentDepthCounter", SubagentDepthCounter);

/// Host-injected max nesting depth; absent → [`super::MAX_SUBAGENT_DEPTH`].
#[derive(Debug, Clone, Copy)]
pub struct MaxSubagentDepth(pub u32);

register_resource!("distill", "MaxSubagentDepth", MaxSubagentDepth);
...
register_resource!("distill", "TaskModelValidator", TaskModelValidator);
...
register_resource!("distill", "SessionIdResource", SessionIdResource);
...
/// Carries the current parent prompt/turn ID for TaskTool subagent scoping. Set by distill-shell
...
/// True while a `/goal` loop is active. Set by distill-shell at turn start. When true, `TaskCompletionReminder`
...
register_resource!("distill", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("Distill:run_terminal_cmd", ToolKind::Execute),
                tc("Distill:read_file", ToolKind::Read),
                tc("Distill:list_dir", ToolKind::List),
                tc("Distill:grep", ToolKind::Search),
                tc("Distill:kill_task", ToolKind::KillTaskAction),
                tc("Distill:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("Distill:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
