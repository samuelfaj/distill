...
/// form `"namespace.Name"` (e.g., `"distill.ReadFile"`). The ID is used as the serialization key when persisting
/// resources. Use the `register_resource!` macro to implement this.
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"distill.ReadFile"`.
    const ID: &'static str;
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"distill.ReadFile"`).
    id: String,
...
impl Resources {
    ...
    pub fn require<T: Send + Sync + 'static>(&self) -> Result<&T, distill_tool_runtime::ToolError> {
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), distill_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn distill_tool_runtime::ToolDispatch>` so it can be stored in
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn distill_tool_runtime::ToolDispatch>);
#[derive(Debug, Clone)]
...
pub trait ManagedGatewayToolCaller: Send + Sync {
    async fn call_tool(
        ...
        caller: &str,
    ) -> Result<ManagedGatewayToolCallResponse, distill_tool_runtime::ToolError>;
}
#[derive(Clone)]
...
/// [`distill_tool_runtime::ToolCallContext::extensions`] by `prepare_dispatch` / `call_raw` from that
...
register_resource!("distill", "WebCitation", WebCitationCounter);
...
/// (`ListMcpResources`, `FetchMcpResource`) can access MCP servers without depending on `distill-mcp` directly.
...
mod tests {
    ...
    register_resource!("distill", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
        files_read: Vec<String>,
    }
    register_resource!("distill", "ReadFile", ReadHistory);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct TodoData {
        items: Vec<String>,
    }
    register_resource!("distill", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("distill.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "distill.ReadFile".to_string(),
            serde_json::json!({"files_read": ["loaded.rs"]}),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "distill.Edit".to_string(),
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "distill.Edit").unwrap();
        assert_eq!(
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "distill.Edit").is_none());
    }
    #[test]
    ...
    fn resolve_model_path_tilde_expands_to_home() {
        let home = distill_dirs::home_dir().expect("test requires home_dir");
        let cwd = std::path::Path::new("/worktree/abc");
