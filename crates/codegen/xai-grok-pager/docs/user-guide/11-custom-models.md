...
By default, Grok uses models hosted by Samuel Fajreldines, and new sessions start with `grok-4.5`. Default models require no configuration. Authenticate with `grok login` or an API key, then start a session.
