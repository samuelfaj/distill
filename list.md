...
| dois modelos (main, reasoning) + utility | O **main** (`/model`, obrigatório, `[models].default`) roda toda sessão e todo passo. O **reasoning** (`/reasoning-model`, opcional, `[models].reasoning`) nunca assume a sessão: a cada passo o Jev decide se o main dá conta; quando não dá (pedido novo não trivial, arquitetura, evidência a interpretar, recuperação de falha, revisão), o reasoning recebe uma visão limitada e sem ferramentas e devolve um plano que entra na conversa como `<reasoning_advice>`. Na dúvida, um pedido sem conselho ainda é planejado; um pedido já aconselhado segue o plano. `plan` e `code-reviewer` rodam no reasoning. A lane barata (utility) continua em `[jev.local] model`. Configs antigas com `[jev.tiers] light` são migradas no startup: o worker vira main e o default antigo vira reasoning | `jev_reasoning_step` + `reasoning_consult_questions`/`compose_reasoning_consult`, `resolve_subagent_sampling_config`, `migrate_model_roles` | `[models].reasoning` / `b2_reasoning_model` (alias `b2_light_model`) | implemented | `the_main_model_consults_reasoning_only_when_a_step_is_beyond_it`, `an_unsure_consult_plans_new_requests_and_keeps_existing_plans`, `plan_and_review_use_the_reasoning_model_and_other_tasks_stay_on_main`, `the_worker_becomes_main_and_the_old_default_becomes_reasoning` |
