...
| `json_crusher` | Deterministic JSON/TOON crush | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `log_crusher` | Collapse repeated log lines | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `stack_crusher` | Keep frames, drop dumps | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `test_crusher` | Keep fail names and assertion text | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `diff_crusher` | Keep file headers and hunks, drop noise | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `html_crusher` | Strip chrome, keep text | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `source_skeleton` | Signatures/line map without bodies | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `toon_codec` | Uniform JSON arrays as TOON | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers); função pura — ligação ao caminho vivo ainda pendente |
| `notebook_crusher` | Strip .ipynb outputs/base64 images; keep code+markdown cells | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `lockfile_crusher` | package-lock/yarn.lock/Cargo.lock/Package.resolved → top-level deps + counts, never full graph | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `generated_asset_notice` | Binary/minified/generated mega-file → typed notice (size, kind, hash) + handle instead of bytes | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `embedded_blob_crusher` | base64/data-URI/hexdump islands inside text → typed placeholder + sub-handle | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `progress_bar_crusher` | Collapse carriage-return progress/spinner frames (npm, pip, docker pull) to final state per bar | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `secret_redact_view` | Masked view of secret-bearing blob: keys kept, values masked; deterministic masker, fail-closed | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `repo_map_budget` | Ranked repo map of signatures fitted to a token budget (aider-style) for session boot | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers); função pura — ligação ao caminho vivo ainda pendente |
...
| `handle_token_estimate` | Per-provider token estimate + head/tail preview of a handle, so paid can decide retrieve-or-not | pós-processo de tool result | `e_importance` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `identifier_alias_codec` | Reversible aliasing of long UUIDs/hashes/paths to short tokens; expansion table behind a handle | pós-processo de tool result | `e_importance` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `ansi_escape_strip` | Strip ANSI color/cursor escape codes that inflate tokenization | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `padded_table_compact` | Collapse alignment whitespace in columnar CLI output; CSV/TOON re-emit | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `write_ack_verify` | Successful write/edit tool results → terse ack {lines, hash, applied hunks} instead of full echo; kills the verify re-read | pós-processo de tool result | `e_read_reuse` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers); função pura — ligação ao caminho vivo ainda pendente |
...
| `svg_crusher` | Strip SVG path-coordinate blobs → structure, text, viewBox; full behind handle | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
...
| `duplicate_handle_notice` | This blob already stored as handle X | pós-processo de tool result | `e_importance` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
| `secret_presence_flag` | Regex/heuristic: blob looks secret-bearing; do not echo values | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
...
| `compact_span` | Existing compact span codec | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
...
| `pii_presence_flag` | Heuristic PII detector (emails, names, document numbers) → flag + masked view; keeps customer data out of paid context | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
...
| `injection_pattern_flag` | Heuristic prompt-injection detector on fetched/untrusted content → flag + quarantine notice; content stays behind handle until paid opts in | pós-processo de tool result | `e_crushers` | `crushers::tests` (cargo test -p distill-workspace --lib jev::crushers) |
...
| `codex sticky turns` | Keep a turn's rounds on the ChatGPT (Codex) replica that holds their cached prefix: the first response of a turn issues `x-codex-turn-state`, the session's sampler actor sends it back on every later round of that turn (same model), and a new turn starts without it — the Codex client's own contract. Before it, rounds of one turn often read only the shared ~6.6k-token system prefix from cache | sampler actor + Responses stream request | always on (Codex host only) | implemented | `client::tests::codex_turn_state_is_echoed_only_within_its_turn` |
...
| OAuth próprio do ChatGPT | O Distill faz o login no ChatGPT por conta própria — PKCE no navegador com servidor de callback local, ou fluxo de device code quando não há navegador — e guarda o token no próprio arquivo (`~/.grok/codex-auth.json`, dono-only), com refresh proativo e revogação no logout. É o OAuth do open-grok portado inteiro, não o `auth.json` do CLI do Codex: nada de outro programa é lido ou sobrescrito | `codex_auth` (store + login + refresh) e `secure_file` para a gravação | `Distill login --chatgpt` / `logout --chatgpt` | implemented | `authorize_url_matches_codex_rust_contract`, `credentials_read_openai_claim_namespace`, `browser_launch_failure_exposes_manual_authorization_url`, `callback_state_mismatch_does_not_consume_login_attempt`, `storage_is_separate_and_owner_only`, `logout_removes_malformed_auth_store` |
| identidade do provedor no fio | O bearer do ChatGPT vai com os headers de conta derivados das claims do token, e esses headers são *reservados*: o cliente remove do saco estático qualquer versão que a config do modelo traga, e falha fechado quando não há credencial viva, em vez de mandar token velho | `ResolvedBearerAuth` + `reserved_headers` + `fail_closed_on_missing` no sampler, aplicados em `client.rs` | — | implemented | `bearer_resolver_binds_atomic_headers_to_expected_identity`, `permanent_refresh_failure_is_scoped_to_refresh_token_and_account` |
| login por provedor na home | A home lista `Log in with Grok`, `Log in with ChatGPT` e `Log in with OpenRouter`; e a tela de gate (sem sessão Grok) também oferece ChatGPT e OpenRouter, porque o harness roda com qualquer um deles — ou com nenhum, só modelos locais | menu de boas-vindas + `dispatch_menu_action` (mapa do menu de gate) | — | implemented | `menu_action_provider_rows_route_to_login_and_notices`, `menu_action_indices_match_the_shipped_menu` |

| home vertical com o gato | A tela inicial virou coluna: o gato do dono no tamanho em que o script dele pinta (52×30 células, sem reamostrar), o título `Distill` em dourado logo abaixo, e o menu embaixo — tudo alinhado à esquerda. O changelog saiu da home (nem a linha no menu, nem o bloco de texto): as release notes ficam no `/release-notes`, e as linhas que sobraram são do gato | `cat` (meio-bloco truecolor) + layout empilhado + `title_spans` | — | implemented | `the_asset_is_the_scripts_size`, `a_cell_carries_its_two_pixels`, `rendering_paints_every_cell_of_the_grid`, `the_cat_keeps_the_scripts_size`, `legacy_consoles_get_the_ascii_cat` |

| dois modelos (main, reasoning) + utility | O **main** (`/model`, obrigatório, `[models].default`) roda toda sessão e todo passo. O **reasoning** (`/reasoning-model`, opcional, `[models].reasoning`) nunca assume a sessão, e toda consulta é decisão do Jev: **plano** (na primeira rodada: sem plano, plano já, ou plano depois da primeira evidência; o Jev também julga a complexidade), **passo** (a cada rodada com trabalho novo que o reasoning não pesou: o main travou? Falhas, a chamada que mais falhou, a mais repetida, falhas seguidas, edições desfeitas e rodadas sem conselho entram como fatos, não como gatilhos) e **revisão na entrega** (enquanto houver trabalho que nenhuma revisão viu; `VERDICT: revise` mantém o main trabalhando). Sem orçamento, intervalo ou limite de tamanho fixos; resposta insegura ou ausente não gasta nada. Com `/effort auto`, a pergunta da rodada vai no mesmo pedido de decisão do effort do main (uma chamada do Jev por rodada). A edição sinalizada pelo C4 (já uma decisão do Jev) é revisada na hora. As consultas de um pedido são uma única conversa com o reasoning: instruções idênticas, cada consulta reenvia o histórico sem mudança e acrescenta só o trabalho novo desde a última resposta, com um `prompt_cache_key` por pedido (prefixo em cache). Em cada consulta o Jev decide, numa bateria só, que itens novos vão inteiros ou como resumo de uma linha e (com `/effort auto`) o effort daquela consulta; sem resposta, tudo inteiro e o effort configurado. O que não cabe na janela vira citação verificada do utility (`cite_spans`) e depois resumo. O conselho entra na conversa como `<reasoning_advice>`; a linha de status mostra `reasoning <tipo> <modelo> <effort>` enquanto ele trabalha e o relatório do turno lista os tokens dele e `Reasoning - Nx (…)`. Configs antigas com `[jev.tiers] light` são migradas no startup | `reasoning_gates` (regras puras), `jev_reasoning_step`/`jev_delivery_review`, `reasoning_consult_questions`, `migrate_model_roles` | `[models].reasoning` / `b2_reasoning_model` (alias `b2_light_model`) | implemented | `the_main_model_consults_reasoning_only_when_jev_decides`, `a_simple_request_stays_with_the_main_model`, `the_delivery_review_runs_when_jev_wants_it_and_continues_only_on_problems`, `one_decision_request_carries_the_effort_and_the_rounds_reasoning_question`, `step_facts_report_failures_loops_and_undone_edits_since_the_last_advice`, `the_round_asks_the_plan_first_then_only_about_new_work`, `a_delivery_is_reviewable_until_a_review_saw_it`, `only_a_confident_known_answer_is_a_reasoning_pick`, `consults_share_one_thread_and_jev_decides_each_brief_and_effort`, `work_too_big_for_the_reasoning_window_goes_as_its_summary`, `the_thread_resends_its_prefix_and_carries_only_new_work`, `b2_brief_sends_in_full_only_what_the_consult_needs` |

| cadeia de fallback do modelo barato | O modelo da lane barata aceita vários ids do OpenRouter separados por vírgula e o harness manda a lista como rota de fallback (`model` = o primeiro, `models` = os seguintes), então o OpenRouter tenta na ordem e cobra só quem respondeu; o primeiro id é o que o registro nomeia quando a resposta não diz quem serviu | corpo da chamada (`chat_message_body`) e resolução da lane (`cheap_lane`) | `[jev.local] model` (padrão: cadeia do dono) | implemented | `a_comma_separated_model_spec_becomes_a_fallback_chain`, `a_single_model_spec_sends_no_models_key`, `blank_rungs_are_dropped_from_the_chain`, `the_shipped_cheap_spec_is_the_owner_chain_in_order`, `a_standalone_spec_builds_a_lane_on_the_openrouter_defaults`, `a_comma_separated_chain_is_accepted_in_order` |
...
| capacidade | o que faz | seam | flag | status | teste / motivo |
| --- | --- | --- | --- | --- | --- |
| banner do provedor fora | O banner de anúncio que o backend envia não aparece: é marketing de um provedor só, e este harness roteia para vários; a env vence, e sem env a seção decide (ausente = fora); os dois produtores (`x.ai/announcements/update` e o caminho de settings do event loop) passam pela mesma porta, porque uma porta em um só deles é o banner que aparece assim mesmo | `acp_handler::settings::visible_announcements` | `[announcements] enabled` / `DISTILL_ANNOUNCEMENTS=1` | implemented | `a_backend_push_is_dropped_unless_announcements_are_enabled`, `legacy_announcement_env_remains_a_compatibility_fallback` |
| linhas de login no menu | Tela de boas-vindas ganha `Log in with Grok`, `Log in with Codex`, `Log in with OpenRouter` e `Model tiers`; Grok dispara o fluxo real, os outros três abrem o aviso com o estado vivo e o próximo passo exato | menu de boas-vindas + `dispatch_menu_action` | — | implemented | `menu_action_provider_rows_route_to_login_and_notices`, `menu_action_indices_match_the_shipped_menu`, `menu_action_indices_shift_by_the_import_row` |
