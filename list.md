...
| dois modelos (main, reasoning) + utility | O **main** (`/model`, obrigatório, `[models].default`) roda toda sessão e todo passo. O **reasoning** (`/reasoning-model`, opcional, `[models].reasoning`) nunca assume a sessão e só é consultado em pontos de decisão: **plano** (uma vez por pedido; o Jev julga necessidade, intenção e complexidade; trabalho no workspace é planejado depois da primeira evidência), **recuperação** (sinais determinísticos de que o main travou: mesma chamada falhando 2x, 3 falhas seguidas, mesma chamada 3x, edição desfeita, 20 rodadas sem conselho; também a edição sinalizada pelo C4) e **revisão na entrega** (antes de entregar mudanças não triviais; `VERDICT: revise` mantém o main trabalhando). Orçamento por pedido: 1 plano, 2 recuperações, 1 revisão, intervalo de 2 rodadas entre recuperações. As consultas de um pedido são uma única conversa com o reasoning: instruções idênticas, cada consulta reenvia o histórico sem mudança e acrescenta só o trabalho novo desde a última resposta, com um `prompt_cache_key` por pedido (prefixo em cache). Em cada consulta o Jev decide, numa bateria só, que itens novos vão inteiros ou como resumo de uma linha e (com `/effort auto`) o effort daquela consulta; sem resposta, tudo inteiro e o effort configurado. O que não cabe na janela vira citação verificada do utility (`cite_spans`) e depois resumo. O conselho entra na conversa como `<reasoning_advice>`; a linha de status mostra `reasoning <tipo> <modelo> <effort>` enquanto ele trabalha e o relatório do turno lista os tokens dele e `Reasoning - Nx (…)`. Configs antigas com `[jev.tiers] light` são migradas no startup | `reasoning_gates` (regras puras), `jev_reasoning_step`/`jev_delivery_review`, `reasoning_consult_questions`, `migrate_model_roles` | `[models].reasoning` / `b2_reasoning_model` (alias `b2_light_model`) | implemented | `the_main_model_consults_reasoning_only_at_its_decision_points`, `a_simple_request_stays_with_the_main_model`, `the_delivery_review_keeps_the_turn_going_only_when_it_finds_problems`, `struggle_signals_fire_on_repeated_failure_loops_and_undone_edits`, `delivery_review_skips_trivial_work_and_reviews_the_rest_once`, `consults_share_one_thread_and_jev_decides_each_brief_and_effort`, `work_too_big_for_the_reasoning_window_goes_as_its_summary`, `the_thread_resends_its_prefix_and_carries_only_new_work`, `b2_brief_sends_in_full_only_what_the_consult_needs` |
