...
| três tiers (hard, light, cheap) | A chamada tem três degraus: o modelo da sessão (hard), o irmão leve dele (light, opcional) e a lane barata (cheap). O hard é o modelo da sessão (`/model`), o light mora em `[jev.tiers] light` e o cheap em `[jev.local] model` — cada valor tem um dono só. O par hard/light é validado: mesmo provedor, mesmo backend de wire e mesma credencial; e o irmão leve só pega a rodada se a conversa couber na janela dele, porque um round que ele não segura seria cortado no meio — isso não é roteamento, é outra sessão | resolução por rodada (`light_tier` + `same_family`), decisão em bateria única (`micro_tier_questions` + a de effort) e swap do config da rodada | `[jev.tiers] light` / `b2_light_model` | implemented | `the_same_family_rule_takes_a_sibling_and_refuses_a_stranger`, `the_tier_question_offers_both_models_and_a_keep`, `the_tier_pick_defers_when_unsure_or_abstaining`, `tier_status`, `/tiers hard/light/cheap` |
